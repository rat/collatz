...
and Its Convergent Characterization Across Five Independent Vocabularies}
...
are able to test. We show, by four methodologically independent
vocabularies (endogeny itself counting as the first), that closing
this gap is equivalent to one and the same unresolved arithmetic
statement: Wirsching's 1998 Weak Covering Conjecture
\cite{Wirsching1998} and Tao's 2020 $\beta=1$ conjecture
\cite{Tao2020} (which we show is literally the same algebraic object,
hence counted once), a functional equation for an explicit base-$3$
...
\cite{Tao2022,Tao2011} (also counted once, since both share the same
second-moment Fourier decomposition), both of which we execute to
completion and both of which fail through the \emph{same} missing
ingredient: one
...
We argue that this convergence of five methodologically independent
and technically unrelated vocabularies onto the same unproven
statement --- two of whose pairwise coincidences are themselves
proved identities, not analogies, and are counted once each --- is
itself significant evidence that the obstruction is a genuine feature
of the problem rather than an artifact of any single method, and we
state precisely what remains to be proved.
\end{abstract}
...
\item \textbf{A literature-theoretic contextualization
  ...
  subgroups of $\mathbb{Z}_3^\times$), and we identify a fifth,
  purely combinatorial, independent vocabulary for the same
...
Conditional on the standard extension of the Crump--Mode--Jagers
renewal theorem \cite{Nerman1981} to general, not necessarily
one-sided, reproduction measures --- whose hypotheses we do not verify
in detail here, our branching displacements including negative values
(e.g.\ $2/3<1$ at $q=3$, since a child can be numerically smaller than
its parent) --- the sharper asymptotic $N(x)\sim W\cdot x^{\alpha_-(q)}$
holds almost surely, uniformly and \emph{including} at $q=3$, for an
almost surely positive, non-degenerate random scale factor $W$ (not a
deterministic constant at any $q$, since the underlying reproduction law
has genuine variance). The non-lattice hypothesis of \cite{Nerman1981}
itself holds here, since $\log q/\log2$ is irrational for every $q\ge3$.
\end{proposition}
...
not merely for the i.i.d.\ model of Theorem~\ref{thm:transition-model}
and Proposition~\ref{prop:transition-fine}: for $q=3$ it has positive natural
density ($N_u(x)\sim x$); for every odd $q\ge5$ it has natural density
zero, with $N_u(x)\sim W_u\cdot x^{\alpha_-(q)}$. At $q=3$ this
\emph{implies, and strictly refines}, the Growth Exponent Conjecture of
...
Collatz reverse tree, which asserts only the exponent
$\pi_a(x)=x^{1+o(1)}$, not a specific asymptotic constant --- open
...
rather than resolving it; the further claim of a specific limiting
constant is additional content of Conjecture~\ref{conj:transition-arithmetic}
beyond the classical conjecture.
\end{conjecture}
...
near-independence of fresh digits across sibling subtrees, here in a
quantitative large-deviations form rather than a qualitative one), and
...
setting --- is near-independence, across sibling subtrees, of the
fresh $3$-adic digits of the leaf integers, together with
equidistribution of the Archimedean phase along paths. The known
...
in particular it does not directly bound the exact, coarse,
$\Delta$-dependent pair correlation of
Proposition~\ref{prop:exact-endogeny} below, which has a different
functional form (a fixed correlation coefficient on a coarse
$\sigma$-algebra, not a tunable intensity on full subtree content) and
is not necessarily well-represented by Arm~2's calibration curve; see
Remark~\ref{rem:calibration-consistency} for why the two results are
nonetheless consistent rather than in tension. Equation~\eqref{eq:rho-eff}
...
\begin{theorem}[Regime 3 is at least as hard as recognized open problems]\label{thm:regime3}
Regime 3 above is a special case of, and in the generality required
here is at least as hard as, each of: (i) correlations of digital
functions in bases $2$ and $3$ \cite{Spiegelhofer2021}, (ii) effective
rigidity for $\times 2,\times 3$-invariant structures in the sense of
Furstenberg (the pair $(w_1,w_2)$ is $v$ observed through two elements
of the affine semigroup generated by $x\mapsto(2^ax-1)/3$), open in the
generality required here, and (iii) exponential Fourier decay for
$3$-adic self-similar measures of Breuillard--Varjú type, known only in
special algebraic cases. None of these furnishes a usable tool today.
\end{theorem}
...
explicit. Proposition~\ref{prop:exact-endogeny} concerns the
\emph{correlation coefficient} of the coarse ($\ell=1$, mod $3$)
projection of a \emph{single} sibling pair: exact, nonzero, and
$D$-independent. \S\ref{sec:calibration} measures instead the
\emph{relative} residual variance of full integer population counts,
aggregated over the whole tree at large truncation. The coarse
covariance $C_{\mathrm{exact}}(\ell_0;\Delta)$ above is finite and
$D$-independent, while the aggregate means $\mathbb{E}[Z_i]$ grow
exponentially with truncation depth; hence its normalized contribution
$C_{\mathrm{exact}}/(\mathbb{E}[Z_1]\mathbb{E}[Z_2])\to 0$ as depth
increases. A per-pair coarse correlation that is exactly persistent
therefore contributes a vanishing \emph{fraction} of the aggregate
variance --- real, but below the detection threshold of the
calibration experiment --- and the \emph{rate} of that dilution is
plausibly the observed signal: the excess of \S\ref{sec:calibration}
decays monotonically to zero with truncation depth, as a fixed
$C_{\mathrm{exact}}$ diluted by exponentially growing variance would.
Separately, the refutation of the $L^2$ hypothesis below
(Theorem~\ref{thm:l2-refutation}) is the failure of an \emph{upper
bound} on the fine covariance ($K_\infty=\infty$ makes the bound above
vacuous), not a proof that the fine covariance is in fact large;
\S\ref{sec:calibration} measures directly the fine coupling this bound
leaves uncontrolled, and finds it small. Finally, the geometric branch
weights $q\cdot 2^{-a}$ concentrate on small gaps $\Delta$,
predominantly in the case $\Delta\not\equiv 0\pmod 6$ (coarse
correlation $-1/2$) rather than $\Delta\equiv 0\pmod 6$ (coarse
correlation $+1$), so the net coarse contribution is plausibly
\emph{negative} --- consistent with, though we stop short of claiming
to predict, the observed negative sign of the residual excess in
\S\ref{sec:calibration}. The two sections are therefore complementary
rather than in tension: Theorem~\ref{thm:barrier} and
...
structure survives at every depth), while \S\ref{sec:calibration}
establishes that the actual aggregate residual is nonetheless
empirically small.
\end{remark}
...
as the $L^2$ condition (both decompose via the character sum
...
\subsection{A fifth independent vocabulary, empirically tested}
...
This gives a fifth, technically unrelated, vocabulary for the same
missing ingredient: equidistribution/balance of residues along the
true orbit, appearing independently via the smoothing-transform/
endogeny route (\S\ref{sec:endogeny}), the entropy count shared by the
Weak Covering Conjecture and $\beta=1$ (\S\ref{sec:wcc},
\S\ref{sec:triangulation} --- proved to be the same algebraic object by
Proposition~\ref{prop:beta-wcc}, hence counted once), Wirsching's 2003
functional equation (\S\ref{sec:triangulation}), the second-moment
Fourier analysis shared by the $L^2$ condition and the spectral
dichotomy of Proposition~C (\S\ref{sec:lemmas}, likewise counted once),
and now a purely combinatorial reformulation.
...
Second, these results do not stand alone: the same underlying
...
the crux reached by five methodologically independent vocabularies:
(1) the smoothing-transform/endogeny route (\S\ref{sec:endogeny}); (2)
the entropy count shared by Wirsching's Weak Covering Conjecture and
Tao's $\beta=1$, which Proposition~\ref{prop:beta-wcc} shows to be the
\emph{same algebraic object}; (3) Wirsching's 2003 real functional
...
$\Cov\propto\sum_\xi S_1(\xi)S_2(\xi)^\ast$; and (5) the purely
combinatorial residue-balance reformulation of \cite{Chang2026}. We
count five rather than seven deliberately: two of the pairwise
linkages are not analogies but \emph{proved identities} ---
$\beta=1\cong$~WCC (Proposition~\ref{prop:beta-wcc}) and the common
second-moment origin of the $L^2$ and spectral-dichotomy branches ---
and we count each such pair once. That two vocabularies can be
\emph{proved} to be one object is itself stronger evidence that the
obstruction is a single structural feature of the problem than a loose
analogy would be. A barrier visible from only one angle is a candidate
artifact of that technique; a barrier reached by five independent
vocabularies (branching-process fixed points, integer
covering/entropy counting, a real functional equation, second-moment
character sums, and elementary residue counting), two of whose pairwise
coincidences are theorems, is much better evidence that the obstruction
...
quantification of the size and shape of that wall from five
...
\item \textbf{Near-independence of fresh $3$-adic digits across
  sibling subtrees} (Theorem~\ref{thm:barrier},
  Remark~\ref{rem:barrier-reading}): the arithmetic statement whose
  idealized (i.i.d.) analogue is provable via smoothing-transform
  classification and Choquet--Deny rigidity.
...
\item \textbf{The density transition on the genuine arithmetic tree}
  ...
  Proposition~\ref{prop:transition-fine}); for $q=3$ this conjecture
  implies, and strictly refines, the Growth Exponent Conjecture of
  Kontorovich--Lagarias and Applegate--Lagarias, open since 1995, of
