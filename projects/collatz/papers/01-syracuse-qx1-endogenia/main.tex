...
independent of the digit dynamics, unchanged to every child. For
non-constant $Y:\mathcal{Y}\to[c,C]$ with fixed constants
...
\item the same linear regression slope $b$ of $\log X$ against any
  digit-measurable statistic $Z$ (in particular, against $-\log\mu$),
  up to a fixed additional dispersion;
...
from the branch weights of the recursion alone; $Y$ is carried
unchanged from parent to child and never enters a weight, so
multiplying any solution by $Y$ leaves the weight structure, and hence
the roots $\alpha=1,\alpha^\ast$, untouched. (b) Since $0<c\le Y\le C$
pointwise, $cW\le X\le CW$, so $\Pr(W>x/c)\le\Pr(X>x)\le\Pr(W>x/C)$ for
every $x$: whatever regularly-varying tail index $W$ has, $X$ has the
same index, with no independence hypothesis needed for this alone.
Since $Y$ is in fact independent of $W$ by construction and bounded,
hence $\mathbb{E}[Y^\alpha]<\infty$, Breiman's lemma
\cite{Breiman1965} sharpens this to the exact asymptotic
$\Pr(X>x)\sim\mathbb{E}[Y^\alpha]\Pr(W>x)$: the tail \emph{index} is
preserved exactly, the tail \emph{constant} generally is not. (c) For
any $Z$ measurable with respect to the digit $\sigma$-algebra,
$\log X=\log W+\log Y$ with $Y$ independent of that $\sigma$-algebra
gives $\Cov(\log X,Z)=\Cov(\log W,Z)+\Cov(\log Y,Z)=\Cov(\log W,Z)$, so
the slope $b=\Cov(\log X,Z)/\Var(Z)$ is unchanged; $\Var(\log
X)=\Var(\log W)+\Var(\log Y)$ adds pure noise around the same line.
(d) The digit dynamics never touches the second factor, so
conditioning on any finite or infinite number of digits leaves $Y$,
and hence $\Var[\log Y]$, untouched.
\end{proof}
