...
a companion paper establishes in full \cite{PressureCompanion}; the
rest of this paper depends on the objects and facts stated here.
...
Conditional on a renewal-type refinement (spine decomposition and
exponential tilting for the two-sided random walk underlying the
model, not yet carried out in this exact form; see the companion paper
for the gap in the literature), the counting function has the
sharper asymptotic $N(x)\sim W\cdot x^{\alpha_-(q)}$ almost surely and
...
strictly decrease from the total cost $A_\ell$ down to $0$ as $m$ runs
from $1$ to $\ell$: exactly the strictly-decreasing-exponent sum
defining $R_{j,k}$. Padding the shift to any fixed budget $\ell+j\ge
A_\ell$ instead of the tuple's own cost $A_\ell$ turns this
correspondence into the stated bijection with $R_{\ell-1,j}$; taking
...
(\texttt{sec9-wcc-beta-bridge/weighted\_bridge.py}).
\end{proof}
...
the accompanying repository, \texttt{sec5-rho-eff-control-experiment}:
consecutive children's residues cycle deterministically through all
...
This paper's own claims (\S\ref{sec:endogeny} onward, excluding the
pressure-equation facts restated without proof from
\S\ref{sec:pressure} and the results restated from \S\ref{sec:kl} and
