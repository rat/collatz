...
\cite{PressureCompanion}. Its nontrivial root equals $2$ at $q=3$,
reproducing the Growth Exponent Conjecture of Kontorovich--Lagarias and
Applegate--Lagarias, and falls below $1$ for every $q\ge5$: a
structural transition, not a small perturbation. The root is always
unfrozen, in the sense of directed polymers in a random environment, so
the identity transfers rigorously to the matching i.i.d.\
branching-random-walk model, and, as empirical evidence rather than
proof, to the arithmetic tree itself.
...
$\alpha^\ast(9)=0.258108$: a structural transition at $q=5$,
not a small perturbation. At $q=3$ the second root
$\alpha^\ast=2$ matches the universal tail exponent for the classical
Collatz reverse tree, established in earlier, classical-map-only work
of this project by a Hill estimator and, independently, by
extreme-value theory (not detailed in this paper), and reproduces the
Growth Exponent Conjecture of
Kontorovich--Lagarias \cite{KontorovichLagarias2009} and
Applegate--Lagarias
\cite{ApplegateLagarias1995I,ApplegateLagarias1995II}, open since
1995, as the $\alpha_-=1$ case of a single closed-form equation across
every odd $q$.
...
population of $q$-adic residues, not a sample) remains inconclusive.
...
to exponent $0.650919$. The separation widens, not narrows, as bias is
calibrated away with depth. This measurement compares against the
exponent value $0.678$, not against Volkov's branching model itself,
...
$\beta=1$ (identity and bound checked against brute force). The
...
corroborating evidence, not a decisive test.
...
residue coordinate $a\in\mathbb Z_3^\times$ that $(\star3)$ needs, a
reading that corrects how this project's own earlier attempt on
Conjecture 2 had been framed.
...
direction opposite to what $(\star3)$ needs (a lower, not upper, bound on
every residue).
\end{theorem}
...
absolute summation of the finite Fourier series does not establish
uniform positivity on this range, not that it fails.
\end{empirical}
...
whether sibling subtrees' \emph{content} is shared, not whether their
root residues correlate. The bound $\rho_{\mathrm{eff}}\lesssim0.06$
...
route, not the fine covariance itself, which \S\ref{sec:calibration}
measures directly and finds small. Weighting branch pairs by their
...
from ours ($\times2\bmod3^s$ at finite depth, not $\times3/2\bmod1$ on
an infinite orbit). Ours anchors instead in the Erdős/Lagarias family,
...
condition, of the same missing ingredient, not a methodologically
independent fourth or fifth route in its own right.
...
decay rate are generic, not pathological, in the class of
...
Conjecture, needs the specific arithmetic input of this problem, not
generic smoothness. It should not be over-interpreted as evidence
...
\item \textbf{Wirsching's Conjecture 2} (\S\ref{sec:triangulation}),
  ...
  a previously proposed sufficient condition for it failed for exactly
  this reason, not the one originally supposed
  \cite{WirschingCompanion}.
...
\item \textbf{Absolute continuity and the tail index of the tilted
  ...
  exponent, not on Conjecture~\ref{conj:tail-index} directly.
...
\item \textbf{The exponent transition on the actual arithmetic tree}
  ...
  controls \cite{KLVolkovCompanion}; this is evidence, not a proof, and
  it targets the disputed exponent value rather than Volkov's branching
  model itself.
\end{enumerate}
