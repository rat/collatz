...
than the agreement being claimed). A subsequent, substantially larger
test (5000 roots, four headroom levels up to $10^8$, bootstrap
confidence intervals, and an independent rank-size regression;
\texttt{collatz-endogeny/sec3-pressure-equation}) is encouraging but
still not decisive: the estimate is remarkably stable across headroom
levels, and at the tail fraction judged most balanced (5\%) it lands
at $1.58$ (95\% CI $[1.41,1.80]$), close to the predicted $1.536$ ---
but, as is typical of Hill-type estimators, the point estimate is
unstable across the choice of tail fraction (from $1.39$ at 10\% to
$2.10$ at 1\%), so we do not treat this as confirmation. We are not
aware of any fully decisive empirical test of
Conjecture~\ref{conj:tail-index} for $q\ge5$ (e.g.\ a pre-registered
Clauset--Shalizi--Newman-type comparison against alternative tail
models), and list it as an explicit open problem in
...
\item \textbf{The tail index of the martingale scale factor}
  ...
  been shown to be statistically non-confirmatory, and a much larger
  follow-up test (\S\ref{sec:freezing}) is encouraging at one tail
  fraction but unstable across others, hence still not decisive. This
  is, honestly, the weakest-supported claim in the paper, and we flag
  it as such rather than let it inherit credibility from the $q=3$
  case merely by sharing a formula.
\end{enumerate}
