...
$q\ge5$, signalling a qualitative change from positive to zero natural
density of the reverse tree. We show, moreover, that the two roots of
this equation always sit on opposite sides of a quenched/annealed
freezing transition in the sense of directed polymers in a random
environment: the smaller root is always unfrozen, so the annealed
identity transfers rigorously to the real, quenched counting function;
the larger root is always frozen, so it does \emph{not} by itself
...
Theorem~\ref{thm:transition} below (the smaller root $\alpha_-(q)$,
...
\begin{theorem}[Structural transition: density]\label{thm:transition}
For $q=3$ the reverse tree of $T_q$ has positive natural density
(counting function $N_u(x)\sim x$); for every odd $q\ge 5$ it has
natural density zero, with $N_u(x)\sim W_u\cdot x^{\alpha_-(q)}$ for
some random scale factor $W_u$, $\alpha_-(q)<1$.
\end{theorem}

By Proposition~\ref{prop:always-frozen}, $\alpha_-(q)$ is always
unfrozen, so the annealed identity of Theorem~\ref{thm:pressure}
transfers to the real, quenched counting function at this root,
consistently with the direct empirical confirmation below and with the
statistically calibrated measurement of \S\ref{sec:kl}.
...
model: the tail-index equation $\mathbb{E}[\sum_i A_i^s]=1$ becomes,
...
We state this as a conjecture, not as part of
Theorem~\ref{thm:transition}, precisely because
Proposition~\ref{prop:always-frozen} shows $\alpha_+(q)$ is
\emph{always} frozen: the annealed pressure identity does not
establish it for any $q$, and it must rest on evidence external to
...
reached it. Given the previously established slow convergence rate for
$q=5$ (a subdominant complex root giving $k^{-0.222}$ decay,
\S\ref{sec:freezing}), the transient decays by only about $7\%$ between
...
already settled in \S\ref{sec:regimes}, and resolving it will likely
require either an analytic handle on the transfer operator's
subdominant spectrum or a pre-registered comparison against alternative
tail models on a much larger computational budget than exhaustive
population enumeration allows. We list it as an explicit open problem
in \S\ref{sec:conclusion}.

The mean drift $\log q - 2\log 2$ changes sign exactly at the
threshold $q=5$ separating the two cases of
Theorem~\ref{thm:transition}: negative for $q=3$ (orbits contract on
...
Proposition~\ref{prop:always-frozen}: the Goldie/Guivar'ch regime
...
\item \textbf{The tail index of the martingale scale factor}
  ...
  $q=3$ case, this has neither a rigorous derivation (the relevant root
  is always frozen, Proposition~\ref{prop:always-frozen}) nor solid
  empirical support: the original measurement, for $q=5$, has since
  been shown to be statistically non-confirmatory, a full battery of
  four estimators (\S\ref{sec:freezing}) gives a mixed rather than
  confirmatory picture, and an exact population-level moment test on
  the same section is inconclusive at the depth exhaustive enumeration
  can reach, for a specific and identified reason (a known
  slowly-decaying transient). This is, honestly, the weakest-supported
  claim in the paper, and we flag it as such rather than let it inherit
  credibility from the $q=3$ case merely by sharing a formula.
\end{enumerate}

Any progress on any one of (O1)--(O7) would very likely transfer, given
...
experimentation, and manuscript preparation. The reference to Halász
\cite{Halasz1977} could not be verified directly, as the original 1971
work was not located; only its 1972 erratum was found. For this
reason, the citation is retained with the appropriate caveat regarding
the inability to consult the primary source. We follow, in this
disclosure, what we believe is best practice in the small existing
literature on AI-assisted contributions to this specific problem.
