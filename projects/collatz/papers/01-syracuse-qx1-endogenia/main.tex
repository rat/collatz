...
\author{Renato Augusto Tavares~{\small\href{mailto:dr.renatotavares@gmail.com}{\textcolor{black}{\faEnvelope}}}~{\small\orcidlink{0009-0002-0196-3311}}\\
...
We derive a closed-form pressure equation $\rho(M_q(\alpha)) =
q^{\alpha-1}/(2^\alpha-1)$ governing the tail exponent of the natural
multiplicative population functional on the reverse tree of the
accelerated $qx+1$ map, and show that its nontrivial root undergoes a
structural transition at $q=5$: it equals exactly $\alpha^\ast=2$ at
$q=3$, matching the universal tail exponent established empirically in
earlier work on the classical Collatz map, but falls strictly below
$1$ for every $q\ge 5$, signalling a qualitative change from positive
to zero natural density of the reverse tree. We then identify and
...
Richardson extrapolation to a sequence of nested-window measurements,
...
\item \textbf{An unconditional empirical result (\S\ref{sec:kl}).} We
  resolve, using exact enumeration and Richardson extrapolation, a
  fifteen-year-old dispute between two competing predictions for the
...
given parent $u$ depends only on $u\bmod q$: there is a unique residue
$a_0(u)\in\{1,\dots,d\}$ with $2^{a_0(u)}u\equiv 1\pmod q$, and the
admissible exponents are exactly $a\equiv a_0(u)\pmod d$. Nodes with
$u\equiv 0\pmod q$ are \emph{sterile} (no admissible children); for
$q=3$ this recovers the familiar fact that multiples of $3$ have
trivial reverse subtrees.
...
\subsection{The pressure operator and its closed form}

Let $M_q(\alpha)$ denote the (infinite, but row/column-finite in the
relevant sense) transfer operator on residue classes mod $q$ whose
matrix entries are the $\alpha$-th powers of the branch weights in
\eqref{eq:G-recursion}; concretely, the entry from parent-residue $r$
to child-residue $r'$ collects the weight $(q\cdot 2^{-a})^\alpha$ for
each admissible $a$ mapping $r$ to $r'$. Because admissibility depends
on the parent only through $u\bmod q$ and cycles through a single
residue class of $a$ modulo $d=\ord_q(2)$, the total outgoing weight
from a fixed parent residue is
\[
  \sum_{k=0}^{\infty} \bigl(q\cdot 2^{-(a_0+kd)}\bigr)^{\alpha}
  = \frac{q^{\alpha}\, 2^{-\alpha a_0}}{1-2^{-\alpha d}},
\]
a priori dependent on $a_0=a_0(u\bmod q)$. The key structural fact,
verified independently by exact bisection for $q\in\{3,5,7,9\}$ below
and derived from the bijective structure of the admissibility map (for
each fixed $a$, the map $u\bmod q^k\mapsto w_a\bmod q^k$ is injective
on its domain of definition, a consequence of $2$ being invertible
modulo $q^k$), is that the column sums of $M_q(\alpha)$ --- not the row
sums --- are constant across residue classes, so that the Perron
eigenvalue (spectral radius) of $M_q(\alpha)$ admits the closed form
\begin{equation}\label{eq:pressure-closed-form}
  \rho\bigl(M_q(\alpha)\bigr) = \frac{q^{\alpha-1}}{2^\alpha-1}.
\end{equation}

\begin{theorem}[Pressure equation]\label{thm:pressure}
The tail exponent $\alpha$ of the population martingale $G$ on the
reverse tree of $T_q$ satisfies $\rho(M_q(\alpha))=1$, equivalently
...
Equation~\eqref{eq:pressure-closed-form} evaluates to exactly $1$ at
$\alpha=1$ for every $q$, since $q^0=1=2-1$. Thus $\alpha=1$ is always
a root of \eqref{eq:pressure-eq}, consistently with mass conservation
of the branching process (at $\alpha=1$, $M_q(1)$ is a genuine
stochastic-type operator on population counts).
\end{remark}
...
\begin{theorem}[Structural transition]\label{thm:transition}
For $q=3$ the reverse tree of $T_q$ has positive natural density
(counting function $N_u(x)\sim x$); for every odd $q\ge 5$ it has
natural density zero, with $N_u(x)\sim W_u\cdot x^{\alpha_-(q)}$,
$\alpha_-(q)<1$, where $W_u$ is a random tail-index-$(\alpha_+/\alpha_-)$
scale factor.
\end{theorem}

The mean drift $\log q - 2\log 2$ changes sign exactly at this
threshold: negative for $q=3$ (orbits contract on average), positive
for $q\ge 5$ (orbits expand on average), consistent with
...
\S\ref{sec:setup}, confirms Theorem~\ref{thm:transition} directly:
...
numerically identical to, our pressure-equation derivation. What
survives as new is the closed-form equation~\eqref{eq:pressure-eq}
itself, unifying the family for arbitrary $q$ in a single algebraic
identity, together with the rigorous derivation and independent
numerical confirmation reported here.
\end{remark}
...
$m'>0$} --- a Goldie/Guivar'ch-type regularly-varying tail regime
...
Richardson extrapolation above).

\begin{theorem}[Empirical resolution of the Kontorovich--Lagarias vs.\
Volkov dispute]\label{thm:kl}
The $5x+1$ reverse-tree counting exponent, measured by exact
enumeration with Richardson-extrapolated truncation correction, is
...
\begin{theorem}[Direct computational test of the Weak Covering
...
\begin{theorem}[Numerical test of Wirsching's Conjecture 3]
...
with a cauda-shape systematic uncertainty of $\pm 0.015$, giving
$c=e^L\approx 0.538$ (honest range $0.53$--$0.55$), best fit by a
cauda term $0.79/\sqrt\ell$; no log-periodic modulation is detected.
...
Together, Theorems~\ref{thm:wcc}, Proposition~\ref{prop:beta-wcc}, and
\ref{thm:conjecture3} give three independent formal ``roupagens'' ---
...
p(1-p)^{g-1}$ ($\sum_g w_g=1$). Then, exactly,
...
Applying Theorem~\ref{thm:jensen} with $p=p_c=1/\gamma_c$ (the tilted
model matching the Weak Covering Conjecture's threshold slope) gives
$\Lambda = \log\gamma_c \approx 0.5834+O(c)$, confirmed by Monte Carlo
...
($j^\ast(\ell)/\ell\approx1.2$, Theorem~\ref{thm:wcc}).
...
(Theorem~\ref{thm:kl}) whose validity does not depend on any of the
above framing at all: a direct, falsifiable, statistically calibrated
empirical measurement resolving a fifteen-year-old dispute in the
...
quantification of the size and shape of that wall from six independent
angles, and one hard, unconditional empirical result obtained along
the way.
...
Any progress on any one of (O1)--(O6) would very likely transfer, given
...
This research was conducted with the assistance of AI systems: Claude
(Anthropic), used as an interactive research collaborator for
literature search, formal derivation, numerical experimentation, and
manuscript preparation, and an additional large-language-model
consultant (referred to internally as ``Fable'' during the research
process) used for targeted mathematical review and derivation checks
throughout the project. Every claim in this paper that originated from
an AI consultation was independently checked against a primary source,
an independent numerical computation, or both, before being included.
A batch of citations that initially reached us via AI-mediated
literature review (\cite{Halasz1977,TaoVu2009,NguyenVu2011,Mahler1968,
FlattoLagariasPollington1995,Erdos1979,Lagarias2009}) has since been
checked against primary sources: \cite{TaoVu2009,NguyenVu2011,
Mahler1968,FlattoLagariasPollington1995,Erdos1979,Lagarias2009,
Bremont} were obtained and verified directly (in particular, the
precise Z-number definition attributed to \cite{Mahler1968}, the
attribution of the 1979 conjecture to \cite{Erdos1979}, and the exact
statement of Theorem~2.5 attributed to \cite{Bremont} all matched
exactly), and \cite{GoncalvesGreenfeldMadrid2022} was corrected after
this process revealed that its title, as originally reconstructed from
an AI consultation, was simply wrong. One citation,
\cite{Halasz1977}, remains genuinely unresolved: the year originally
attached to it (1977) was itself an error (the correct year is 1971),
and an extensive search --- including a dedicated attempt to locate an
open digitization (HathiTrust, the Internet Archive, JSTOR, and
Hungarian institutional archives) --- did not turn up the primary
1971 text; only a 1972 erratum/remarks note by the same author was
obtained. The bibliographic entry itself (title, year, journal,
volume, and pages) is independently corroborated by multiple secondary
listings (library catalogs and citing articles), so we retain the
citation with the corrected year, while flagging that the primary text
has not been read by us and recommending a reader verify the specific
technical content independently before relying on it. We consider the
whole episode --- one wrong year caught, one wrong title caught, one
theorem statement confirmed verbatim, and one primary source that
simply could not be located --- good evidence for why this
verification pass was worth doing, and an honest illustration of its
limits. We follow, in this disclosure,
what we believe is best practice in the small existing literature on
...
\item \emph{Wirsching 2003 moment recursion} (Theorem~\ref{thm:conjecture3}):
...
\bibitem{GoncalvesGreenfeldMadrid2022} F.\ Gon\c{c}alves, R.\
  ...
  bounded orbits}, arXiv:2111.06170, 2022. [Verified directly against
  the archived PDF (title/authors corrected from an earlier,
...
\bibitem{Bremont} J.\ Br\'emont, \emph{Self-similar measures and the
  ...
  973--1004. arXiv:1910.03463. [Verified: Theorem 2.5, cited in
  \S\ref{sec:context}, confirmed against the full text.]
...
\bibitem{Halasz1977} G.\ Hal\'asz, \emph{On the distribution of
  ...
  (1971), 211--233. [Bibliographic data independently corroborated by
  secondary sources; primary text not located despite a dedicated
  search of open digitization repositories. Only a related 1972
  erratum/remarks note by the same author has been obtained and
  checked. See Acknowledgments.]
...
\bibitem{Erdos1979} P.\ Erd\H{o}s, \emph{Some unconventional problems
  ...
  67--70. [Verified: located directly in the reference list of
  \cite{Lagarias2009}, item [4].]
