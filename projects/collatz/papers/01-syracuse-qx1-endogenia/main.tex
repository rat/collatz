...
$Y$ any bounded factor invariant along the tree, solve the same
recursion as the canonical solution $W$ and match it in pressure, tail
index, and regression slope against the limiting measure. The
...
base-$3$ Fabius function and treats the second, with a corrected
reading of its actual target \cite{WirschingCompanion}; we test the
third to certified numerical error. Two candidate lemmas from Tao's bivariate
...
Wirsching's first 2003 conjecture and treats the second, both
restated without proof in \S\ref{sec:triangulation}, which also tests
the third numerically \cite{WirschingCompanion}. Section~\ref{sec:kl} restates a companion
...
for the precise gap in the literature), the counting function has the
...
against $\mu$ distinguishes $W$ from $W\cdot Y$: this is item (d) above.
...
its companion paper below) are verified in public scripts, organized
by section, at
