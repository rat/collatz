...
than the agreement being claimed). We are not aware of any other
direct empirical test of Conjecture~\ref{conj:tail-index} for $q\ge5$,
and list it as an explicit open problem in \S\ref{sec:conclusion}.
...
\item \textbf{The tail index of the martingale scale factor}
  ...
  empirical support (the one existing measurement, for $q=5$, has since
  been shown to be statistically non-confirmatory). This is, honestly,
  the weakest-supported claim in the paper, and we flag it as such
  rather than let it inherit credibility from the $q=3$ case merely by
  sharing a formula.
\end{enumerate}
