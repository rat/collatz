...
$q=5$ and $q=7$ gives direct empirical support for
Conjecture~\ref{conj:transition-arithmetic}: counting slopes per decade
of $3$ sampled roots gave $\approx0.62$--$0.66$ for $q=5$ (predicted
$0.650919$) and $\approx0.36$--$0.39$ for $q=7$ (predicted $0.373501$).
A statistically calibrated measurement for $q=5$ is in
\cite{KLVolkovCompanion}.

\begin{remark}[Bibliographic calibration]\label{rem:novelty-109}
Neither the value $\alpha^\ast(5)=0.650919$ nor the qualitative
transition at $q\ge5$ is new, and a later result rigorously excludes
$q\ge5$ in the forward direction for the analogous $p=2$ generalization
\cite{GoncalvesGreenfeldMadrid2022}. Wirsching \cite[Ch.~III, \S5,
...
At the truncation depth where calibrated controls read their own
target exponent to within $0.003$, the arithmetic tree reads $0.64926$
($95\%$ interval $[0.64818,0.65027]$), separated from a control built
to exponent $0.678$ by seven band-widths, and matching controls built
to exponent $0.650919$. The separation widens as bias is calibrated
away with depth; it never narrows. This measurement compares against
the exponent value $0.678$ alone, since Volkov's branching model
...
\sum_{k}2^{-k}g_\ell(k,a), \qquad c_i=2\cdot3^i.
\end{equation}
...
(weighted populations of $w_i$'s subtree, $\eta_i:(\mathbb N_+)^D\to[0,1]$
an arbitrary weight assigned to each descendant path, unrelated to the
scalar constant $\eta$ of $(\star3)$ in \S\ref{sec:triangulation},
truncated to depend on $w_i(v)$ only through its residue mod $3^m$,
precision $m$), with $v$
...
$K_\ell$/Jensen computations, the Chang residue-balance test, and the
