...
(weighted populations of $w_i$'s subtree, truncated to depend on
$w_i(v)$ only through its residue mod $3^m$, precision $m$), with $v$
...
\begin{proof}
Fourier inversion on $\mathbb Z/3^m\mathbb Z$ writes
$\mathbb E[Z_1Z_2]=3^{-2m}\sum_{\xi_1,\xi_2\bmod3^m}S_1(\xi_1)S_2(\xi_2)\,
\mathbb E\bigl[e((\xi_1w_1+\xi_2w_2)/3^m)\bigr]$, expanding each $Z_i$
in its own character sum and averaging the product of exponentials
over the uniform admissible class of $v$. Substituting the affine
relation $w_2=2^\Delta w_1+c_\Delta$ turns the inner average into
$e(\xi_2c_\Delta/3^m)\,\mathbb E[e((\xi_1+2^\Delta\xi_2)w_1/3^m)]$,
...
the same value $c>0$ at both admissible residues, $g(1)=g(2)=c$ (the
two residues play interchangeable roles in the branching rule and are
exchanged by the cyclic type structure verified against real integers
in the accompanying repository, \texttt{sec5-rho-eff-control-experiment}).
