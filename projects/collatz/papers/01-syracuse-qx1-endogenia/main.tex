...
$a_0(u)$ for every $u$), nor the numerical enumeration of
\S\ref{sec:kl} below, whose implementation already excludes exactly
these classes; it is a correction to this structural description only.
...
of \eqref{eq:G-recursion} has an exact annealed pressure identity.

\begin{theorem}[Exact annealed pressure identity]\label{thm:pressure}
For every odd $q\ge3$ and every $k\ge1$, the average of the depth-$k$
partition function $Z_k(\alpha;\cdot)$ over all $q^k$ root residues
...
$\alpha^\ast(9)=0.258108$: a genuine structural transition at $q=5$,
not a small perturbation. At $q=3$ the second root
$\alpha^\ast=2$ matches the universal tail exponent for the classical
Collatz reverse tree established by independent methods (Hill
estimator, extreme value theory) in earlier stages of this line of
work, and reproduces the Growth Exponent Conjecture of
...
$s(\alpha_-)>0$ (\emph{unfrozen}) and $s(\alpha_+)<0$ (\emph{frozen})
unconditionally, for every odd $q\ge3$.

\begin{proposition}[The larger root is always frozen]\label{prop:always-frozen}
For every odd $q\ge3$: $s(\alpha_-)>0$ and $s(\alpha_+)<0$.
\end{proposition}
...
With $\mu_{\alpha,k}$ the law of $F_k$ modulo $q^k$ and
$M_k^{(\alpha)}=q^k\mu_{\alpha,k}$, the projective measure $\mu_\alpha$
...
At $q=3$, Conjecture~\ref{conj:tail-index} has real, threefold
empirical support (Hill estimator, extreme-value theory, and the
counting-exponent confirmation below), all independent of the pressure
argument. For $q\ge5$ the evidence is markedly weaker at small sample
sizes but, after a $20\times$ larger pre-registered follow-up
($10^5$ roots), now shows a clean threshold-stability plateau at the
predicted value and a Vuong test that no longer favors a lognormal
alternative --- the strongest evidence to date, though not a proof (the
martingale is provably still pre-asymptotic at the headroom reached).
...
of sufficiently powerful data, the two predictions differing by only
$\Delta=0.027$.
...
framing-dependent contribution reported here.
...
A direct, unconditional bound on $\beta_{\mathrm{eff}}$ comes from an
inter-level identity on the worst-cylinder mass itself, independent of
the microcanonical machinery above. Write $N_\ell(u):=3^\ell\mu_\ell(u)$
and, for $u$ a unit mod $3^\ell$,
...
where $k_0(y)$ is $y$'s position on this orbit and
$W_\ell(k):=\sum_{j\ge0}4^{-j}N_{\ell-1}(A^jk)$. Since $t_0$ and $k_0$
are compatible between adjacent levels (an elementary but
non-trivial fact, checked exhaustively for $\ell\le8$: zero
exceptions), the scale factors cancel in the ratio and
$R_\ell(y)=W_\ell(k)/W_{\ell-1}(k\bmod3^{\ell-2})$, with numerator and
denominator running over the \emph{same} index $j$ with the same
weights $4^{-j}$ (since $A$ is given by an integer formula, iterating
it commutes with reduction mod $3^{\ell-2}$). Substituting
$N_{\ell-1}(A^jk)=N_{\ell-2}(A^jk')R_{\ell-1}(A^jk)$ exhibits $R_\ell$
as a convex combination of $R_{\ell-1}$ values, which lies between
their minimum and maximum. Full derivation and the exact rational
arithmetic certifying the bound below are in the accompanying
repository.
\end{proof}

At level $L=10$, exact rational arithmetic certifies
$\beta_{\mathrm{eff}}\le1.882712$, improving the previous unconditional
bound $2.306270$ recorded above. The rate saturates: measured in
...
units on the same orbit, uniformly down to $x\sim\exp(-c_0\ell)$; any
fixed exponent in such a bound would already improve
$\beta_{\mathrm{eff}}\le1.88$ toward $1$, via a union bound over the
$\sim3^{\ell-1}$ pairs combined with the exact recursive identity
$W_\ell(k)=N_\ell(k)+\tfrac14W_\ell(A(k))$ used above.

A natural attempt to close it directly --- a $3$-adic Weyl-sum
estimate for the mixed character sums attached to the two pair types of
O2 --- fails for a structural, not merely technical, reason: the
relevant sums achieve the maximal square-root cancellation available at
every primitive frequency, but the operator $T/|G|$ they define is,
in the character basis, the matrix of composition with a bijection of
the underlying group, hence unitary; composing with a bijection is an
$L^2$ isometry, so the route returns exactly the trivial
Cauchy--Schwarz bound at every density, and in particular can never be
nontrivial for the specific pair the union bound needs. No bound
depending only on the two set sizes and spectral data of that bijection
can close O2's remaining gap, however strong the individual character
sum estimates are.
...
conditions $(?1)$--$(?5)$, of which two are theorems proved in
\cite{Wirsching2003} ($(?1)\Rightarrow$ Positive Density,
$(?3)\Rightarrow(?2)$) and three are conjectures. The companion paper
reads the chain directly from the primary source and finds that
$(?3)$ and $(?2)$ are in fact the same inequality (only the
quantifier changes) and that $(?4)$, Conjecture 2's actual target, is
a statement purely about the one-dimensional operator $W_3$ carrying no
information about the residue coordinate the rest of the chain needs
--- a reading that corrects how this project's own earlier attempt on
Conjecture 2 (H-134) had been framed.
...
The second conjecture remains open; it is the most concrete sufficient
...
factors are uniformly bounded below on the central-limit window, $(?3)$
...
$(?3)$ local-limit methods can reach: exact equivalence of ensembles at
...
likelihood bound in the wrong direction for $(?3)$
...
direction opposite to what $(?3)$ needs (a lower, not upper, bound on
every residue).
\end{theorem}
...
prove nor refute the uniform asymptotic lower bound $(?3)$ needs.
\end{empirical}
...
verification (identity checked to $2\times10^{-13}$; ratio never below
...
\item \textbf{Cancellation of centered fine-frequency correlations
  ...
  but the diagonal Fourier coefficient the second-moment route actually
  consumes does cancel under that same average, with the surviving
  resonance confined to the coarse affine mode this direction already
  discards; the gap is that this cancellation is over the free
  arithmetic parameter of the coupling theorem, not over the two path
  indices at a fixed integer, which is what the route needs and remains
  open.
\item \textbf{Wirsching's Weak Covering Conjecture} (Conjecture~\ref{conj:wcc}),
  ...
  inter-level identity independent of this microcanonical machinery;
  the remaining gap to $\beta_{\mathrm{eff}}\to1$ is exactly a pairwise
  anti-concentration inequality, for which the natural $3$-adic
  Weyl-sum approach is shown to fail for a structural (unitarity)
...
\item \textbf{Wirsching's Conjecture 2} (\S\ref{sec:triangulation}),
  ...
  Conjecture~2's actual target, condition $(?4)$, is a statement purely
...
\item \textbf{Exponential control of the fine-frequency tail at depth
  ...
  direction from the one used elsewhere in this paper, not previously
  recorded. Measured through $\ell=15$, the primitive spectrum sits
  at the square-root scale on average but is visibly concentrated, not
  flat, and the sublinear-conductor separation this regime's own
  argument requires removes only an asymptotically vanishing fraction
  of the total mass; O4 does not reduce to O2, since inverting a
  pointwise Fourier bound over $3^r$ frequencies costs a factor
  $3^{r/2}$ above the cylinder scale O2 controls.
\item \textbf{Exclusion of primitively spectrally diffuse Weak Covering
  ...
  inversion point is $\gamma\approx3.31$, not the $\gamma=3$ stated
  above, since the identity behind Theorem~\ref{thm:jensen} requires
  $p\ge\tfrac12$ and degrades strictly below it.
...
\ref{prop:primitive-fibre-energy}, and
Theorems~\ref{thm:multiscale-parseval} and
\ref{thm:logarithmic-collision}.
