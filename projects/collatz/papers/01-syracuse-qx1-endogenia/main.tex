...
member of a complete family of weighted collision criteria. These are
...
\begin{remark}
The proof above applies \cite{Liu2000} to the present weights and
checks the stated hypotheses of that theorem for our parameters
(Appendix, verification script), but the citation itself has not yet
been checked against the primary source. Treat
Theorem~\ref{thm:iid-tail} as pending that check.
\end{remark}
...
$\beta=1$. The entropy implication previously asserted here is invalid.
...
convolution-tail estimate for the first implication.
\end{remark}
...
Public verification scripts for the pressure and freezing computations
of \S\ref{sec:pressure}, the residual-coupling experiment of
\S\ref{sec:calibration}, the Kontorovich--Lagarias vs.\ Volkov gate of
\S\ref{sec:kl}, the Weak Covering Conjecture test of \S\ref{sec:wcc},
the Wirsching Conjecture~3 test, the original $K_\ell$/Jensen
computations, and the Chang residue-balance test are organized by
section at \url{https://github.com/faculdade/collatz-endogeny}. The
  public archive does not yet contain the checks for
  Theorem~\ref{thm:wcc-large-deviation} and
...
\ref{thm:logarithmic-collision}, or the calculations in Empirical
...
Depositing and re-running those
scripts in the archive is a release requirement for this draft.
