...
At $q=3$, Conjecture~\ref{conj:tail-index} has empirical support from
two independent estimators (Hill, extreme-value theory), both
independent of the pressure argument. For $q\ge5$, a sample of $10^5$
roots shows a threshold-stability plateau at the predicted value and a
Vuong test that no longer favors a lognormal alternative, though this
is not a proof: the martingale is provably still pre-asymptotic at the
headroom reached.
The companion paper gives the full battery of estimators, the exact
non-statistical moment test used as a second, independent route, and
the reasons neither route is yet decisive for $q\ge5$
\cite{PressureCompanion}.
