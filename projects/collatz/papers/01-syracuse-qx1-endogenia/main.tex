...
exact. One yields a closed-form structure theorem on sibling-aggregate
correlation; a further condition that would close the remaining gap
shows sustained growth through level $17$ instead of saturating. The other, an exact
...
(\eqref{eq:pressure-closed-form}). Since
...
$3^{-2m}S_1(0)S_2(0)=3^{-2m}\bigl(3^m\mathbb E[Z_1]\bigr)\bigl(3^m
\mathbb E[Z_2]\bigr)=\mathbb E[Z_1]\mathbb E[Z_2]$ (using
$S_i(0)=\sum_rZ_i(r)=3^m\mathbb E[Z_i]$ for $w_i$ uniform on
$\mathbb Z/3^m$), which cancels against the mean product subtracted in
$\Cov$, leaving exactly the sum over $\xi\ne0$ stated.
\end{proof}
...
realization of $Z'$ with $|c|<1$; the boundary case $|c|=1$, hence
$p=1/2$ with $|Z'|=1$, requires every $U_g$ to coincide exactly, a
probability-zero event, so it does not affect the average over $Z'$.
Averaging gives $\mathbb{E}[\log|Z|]=\log p$, i.e.\ \eqref{eq:jensen}.
\end{proof}
...
\item \textbf{Exponential control of the fine-frequency tail at depth
  ...
  direction from the one used elsewhere in this paper; this direction
  is not derived here. A separate
...
Theorem~\ref{thm:iid-tail}, but excluding
Conjecture~\ref{prop:transition-fine}, open in the companion paper
too) are proved and verified in full in a
