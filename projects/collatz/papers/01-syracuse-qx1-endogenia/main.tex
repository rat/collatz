...
% the highest marker density of any section in the paper). This
% abstract summarizes a paper that is now the umbrella of a four-paper
% split; where a result is restated here without proof, the companion
% paper carrying the proof is named.
...
A companion paper proves an exact identity,
$\rho_{\mathrm{ann}}(\alpha)=q^{\alpha-1}/(2^\alpha-1)$, for the
annealed pressure of the natural population functional on the reverse
tree of the accelerated $qx+1$ map, via an exact fibre-counting
bijection \cite{PressureCompanion}. Its nontrivial root undergoes a
structural transition at $q=5$: it equals exactly $\alpha^\ast=2$ at
$q=3$, matching the classical Growth Exponent Conjecture of
Kontorovich--Lagarias and Applegate--Lagarias, and falls strictly below
$1$ for every $q\ge5$. The two roots of the pressure equation always
sit on opposite sides of a quenched/annealed freezing transition; the
smaller root is always unfrozen, so the pressure identity transfers
rigorously to the matching i.i.d.\ branching-random-walk model, and,
on strong but not conclusive empirical grounds, to the actual
arithmetic tree.

We identify and formally characterize an \emph{endogeny barrier}, in
the sense of Aldous--Bandyopadhyay \cite{AldousBandyopadhyay2005},
obstructing any proof that relies only on the linear recursive
structure of the tree: solutions of the form $G=W\cdot Y$, with $Y$ an
arbitrary bounded factor invariant along the tree, solve the same
fixed-point recursion as the canonical solution $W$ and are
statistically indistinguishable from it by every observable we test.
Closing this gap leads to a small collection of unresolved arithmetic
statements, studied here through several complementary routes:
Wirsching's 1998 Weak Covering Conjecture \cite{Wirsching1998} and
Tao's weighted $\beta=1$ conjecture \cite{Tao2020}, which we connect by
an exact cost-budget identity and a large-deviation barrier at the
conjecture's critical cost, and for which we now prove an unconditional
bound $\beta_{\mathrm{eff}}\le1.882712$ from an inter-level cascade
identity on the worst-cylinder mass, improving the previous
unconditional bound of $2.306270$; Wirsching's 2003 functional equation
for a base-$3$ analogue of the Fabius function \cite{Wirsching2003},
whose first conjecture we prove and whose third we test numerically to
certified error, with the remaining second conjecture and a corrected
reading of its actual target given in a companion paper
\cite{WirschingCompanion}; and two candidate lemmas suggested by Tao's
bivariate Fourier-decay technique \cite{Tao2022,Tao2011}, of which one
yields a closed-form structure theorem tested against sustained finite-level
growth through level $17$, and the other an exact Jensen calculation
showing slack by a factor $\approx1.88$ in one annealed $\ell^1$
Fourier budget, a slack we show here persists across the entire wider
family of annealed $\ell^r$ budgets. A diagonalization in
discrete-logarithm coordinates now proves an unconditional collision
growth rate $K_{q,\ell}\ge(q/3)^\ell$ for every odd $q\ge3$, with no
lifting hypothesis, reducing the remaining critical case $q=3$ to a
single affine shifted collision. A companion paper reports a fully
independent, calibrated empirical result on the $5x+1$ counting
exponent, separating Kontorovich and Lagarias's prediction from a
competing one due to Volkov once a raw estimator's own bias is
calibrated out with matched synthetic controls \cite{KLVolkovCompanion}.
Finally, a directed literature search situates the barrier within the
wider theory of Fourier decay for self-similar and self-conformal
measures \cite{BakerBanaji2026, Bremont}, and compares the resulting
obstruction with a recent fixed-modulus orbit statistic
\cite{Chang2026}.
\end{abstract}
...
\item \textbf{A closed-form pressure equation for $qx+1$
  (\S\ref{sec:pressure}).} A companion paper proves that the tail
  exponent controlling the reverse-tree population martingale of the
  accelerated $qx+1$ map satisfies the exact equation
  $q^{\alpha-1}=2^\alpha-1$, with a structural transition at $q=5$
  \cite{PressureCompanion}; \S\ref{sec:pressure} restates the result and
...
\item \textbf{A direct computational test of the Weak Covering
  Conjecture, and an unconditional bound on its cost
  (\S\ref{sec:wcc}, \S\ref{sec:triangulation}).}
  Theorem~\ref{thm:cascade-factor} proves
  $\beta_{\mathrm{eff}}\le1.882712$ from an inter-level cascade identity
  on the worst-cylinder mass.
\item \textbf{A calibrated finite-window comparison of two
  counting-exponent predictions for $5x+1$ (\S\ref{sec:kl}).} A
  companion paper separates Kontorovich--Lagarias's prediction from
  Volkov's competing one once a raw estimator's own bias is calibrated
  out with matched synthetic controls \cite{KLVolkovCompanion};
  \S\ref{sec:kl} restates the calibrated separation.
\item \textbf{Triangulation with Wirsching's 2003 functional equation
  (\S\ref{sec:triangulation}).} We prove Wirsching's first conjecture
  here; a companion paper tests the third conjecture numerically and
  corrects the target of the second \cite{WirschingCompanion}.
\item \textbf{Two candidate routes (\S\ref{sec:lemmas}), and an
  unconditional collision-growth bound.} The first yields a structural
  obstruction and a finite-level test of a natural sufficient
  condition. The second leaves an explicit frequency-stability step
  open, gives an exact Jensen calculation for one annealed Fourier
  budget, and, via a diagonalization in discrete-logarithm coordinates
  (Theorem~\ref{thm:diagonal-collision}), proves
  $K_{q,\ell}\ge(q/3)^\ell$ unconditionally for every odd $q\ge3$.
...
conjectural for the coherent $q$-adic environment.
...
more than $500$ values of $v$ per exponent
(\texttt{collatz-endogeny/\allowbreak sec4-endogeny-barrier}),
confirming the predicted
...
to exponent $0.678$ by ten interval-widths, and matching controls built
...
measurement, and the Weyl-sum obstruction are all verified in
\texttt{collatz-endogeny/\allowbreak sec9-worst-cylinder-cascade}.
...
What survives is a structure theorem. With $w_2 =
...
concerns $\xi(3/2)^n$, a different (though structurally
...
repository. This is verified in
\texttt{collatz-endogeny/\allowbreak sec10-diagonal-shifted-collision}.
...
\item \textbf{Cancellation of centered fine-frequency correlations
  ...
  A separate, ongoing investigation into whether the specific Fourier
  coefficient the second-moment route consumes cancels under a
  different average (over the two path indices rather than over the
  free arithmetic parameter of the coupling theorem) is not reported in
  this paper; O1 remains open at the status established above.
...
\item \textbf{The exponent transition on the actual arithmetic tree}
  ...
  \cite{KontorovichLagarias2009}) by ten interval-widths once the
...
Theorem~\ref{thm:cascade-factor} (\texttt{sec9-worst-cylinder-cascade})
and Theorem~\ref{thm:diagonal-collision}
