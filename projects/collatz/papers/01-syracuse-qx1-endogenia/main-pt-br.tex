...
concebível de acoplamento; em particular não limita diretamente a
correlação exata, grosseira, dependente de $\Delta$, de par da
Proposição~\ref{prop:exact-endogeny} abaixo, que tem uma forma
funcional diferente (um coeficiente de correlação fixo numa
$\sigma$-álgebra grosseira, não uma intensidade ajustável sobre o
conteúdo completo da subárvore) e não é necessariamente bem
representada pela curva de calibração do Braço~2; veja a
Observação~\ref{rem:calibration-consistency} para o porquê de os dois
resultados serem, mesmo assim, consistentes em vez de estarem em
tensão. A equação~\eqref{eq:rho-eff} converte a lacuna puramente
...
A Proposição~\ref{prop:exact-endogeny} concerne ao \emph{coeficiente de
correlação} da projeção grosseira ($\ell=1$, mod $3$) de um \emph{único}
par de irmãos: exato, não-nulo, e independente de $D$. A
\S\ref{sec:calibration} mede, em vez disso, a variância residual
\emph{relativa} de contagens populacionais inteiras completas,
agregadas sobre toda a árvore em truncamento grande. A covariância
grosseira $C_{\mathrm{exato}}(\ell_0;\Delta)$ acima é finita e
independente de $D$, enquanto as médias agregadas $\mathbb{E}[Z_i]$
crescem exponencialmente com a profundidade de truncamento; logo sua
contribuição normalizada
$C_{\mathrm{exato}}/(\mathbb{E}[Z_1]\mathbb{E}[Z_2])\to 0$ conforme a
profundidade cresce. Uma correlação grosseira por-par exatamente
persistente contribui, portanto, uma fração \emph{evanescente} da
variância agregada --- real, mas abaixo do limiar de detecção do
experimento de calibração --- e a \emph{taxa} dessa diluição é
plausivelmente o próprio sinal observado: o excesso da
\S\ref{sec:calibration} converge monotonicamente a zero com a
profundidade de truncamento, exatamente como um $C_{\mathrm{exato}}$
fixo diluído por variância crescendo exponencialmente produziria.
Separadamente, a refutação da hipótese $L^2$ abaixo
(Teorema~\ref{thm:l2-refutation}) é a falha de uma \emph{cota
superior} sobre a covariância fina ($K_\infty=\infty$ torna a cota
acima vazia), não uma prova de que a covariância fina é de fato
...
(verificado em \texttt{collatz-endogeny/sec4-endogeny-barrier/}) ---
confirmando, sob esta ponderação específica, que a contribuição
grosseira líquida é \emph{negativa} e não-desprezível em módulo,
consistente com, embora paremos aquém de afirmar que derivamos
completamente, o sinal negativo observado do excesso residual na
\S\ref{sec:calibration}. (Isso não estabelece, por si só, que a família
de acoplamento do Braço~2 do experimento de calibração é ponderada da
mesma forma; veja a observação de escopo na \S\ref{sec:calibration}.)
As duas seções são,
portanto, complementares em vez de estarem em tensão:
o Teorema~\ref{thm:barrier} e a Proposição~\ref{prop:exact-endogeny}
estabelecem que a recursão \emph{não pode forçar} a decorrelação entre
irmãos (a estrutura grosseira por-par sobrevive em toda profundidade),
enquanto a \S\ref{sec:calibration} estabelece que o resíduo agregado
real é, mesmo assim, empiricamente pequeno.
\end{remark}
