...
(Proposição~\ref{prop:always-frozen}), o crescimento quenched=anelado
vale em toda a região não-congelada \cite{Biggins1992}, e quase
certamente: $N(x)\sim x$ em $q=3$ (densidade positiva, já que
$\alpha_-(3)=1$ exatamente); $N(x)\sim W_u\cdot x^{\alpha_-(q)}$ para
algum fator de escala aleatório q.c.\ positivo $W_u$, para todo $q\ge5$
ímpar ($\alpha_-(q)<1$).
\end{theorem}
...
mais difícil que o caso $q=3$ já resolvido na \S\ref{sec:regimes}.
...
$\Lambda = \log\gamma_c \approx 0{,}5834+O(c)$,
