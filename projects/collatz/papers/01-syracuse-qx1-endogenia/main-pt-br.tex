...
e sua Caracterização Convergente por Cinco Vocabulários Independentes}
...
implicando e refinando estritamente a clássica Conjectura do Expoente
de Crescimento de Kontorovich--Lagarias e Applegate--Lagarias, já que
nossa conjectura também fixa a constante limite, não só o expoente).
Mostramos, além disso, que as duas raízes dessa
...
conjectura explícita, agora substancialmente fortalecida por um teste
estatístico de amostra grande em $q=5$ (embora ainda não provada), e
ainda apenas fracamente sustentada para $q\ge7$. Em seguida
...
Mostramos, por quatro vocabulários metodologicamente independentes (a
própria endogenia contando como o primeiro), que fechar essa lacuna
equivale a uma única afirmação aritmética ainda em aberto: a Weak
Covering Conjecture de Wirsching de 1998 \cite{Wirsching1998} e a
conjectura $\beta=1$ de Tao de 2020 \cite{Tao2020} (que mostramos ser
literalmente o mesmo objeto algébrico, portanto contadas uma vez),
...
\cite{Tao2022,Tao2011} (também contados uma vez, já que ambos
compartilham a mesma decomposição de Fourier de segundo momento), ambos
executados até o fim e ambos falhando pelo \emph{mesmo} ingrediente
faltante: um produz um teorema de
...
Bremont}, mostrando que ela é estruturalmente inacessível a esse
ferramental por causa da rigidez dos subgrupos fechados de
$\mathbb{Z}_3^\times$, e identifica um quinto vocabulário, puramente
combinatório e testável de forma independente, para a mesma obstrução
em trabalho muito recente \cite{Chang2026}. Argumentamos que essa
convergência de cinco vocabulários
metodologicamente independentes e tecnicamente não relacionados ---
duas de cujas coincidências par-a-par são, elas mesmas, identidades
provadas, não analogias, e são contadas uma vez cada --- sobre a
...
\noindent\textbf{Palavras-chave:} conjectura de Collatz; mapa $qx+1$; variável aleatória
de Syracuse; equação de pressão; transição de congelamento; passeio aleatório ramificado;
índice de cauda do martingale; endogenia; Weak Covering Conjecture; função de Fabius;
decaimento de Fourier; expoente de contagem.
...
\item \textbf{Uma contextualização na literatura (\S\ref{sec:context}).}
  ...
  $\mathbb{Z}_3^\times$), e identificamos um quinto vocabulário
  independente, puramente combinatório, para a mesma obstrução em
  trabalho muito recente \cite{Chang2026}.
\end{enumerate}
...
$q=5$, $Z_k(1;u_0)^{1/k}$ nas raízes $u_0\in\{1,2,3,4\}$
igualmente falha em acompanhar $\rho_{\mathrm{ann}}(1)=1$. Sinalizamos,
em vez de esconder, uma limitação desta checagem específica: cada uma
dessas raízes está sobre, ou alimenta diretamente, o ciclo trivial do
próprio mapa de menor ramo admissível ($1\leftrightarrow3$ para $q=5$,
exatamente como $u_0=1$ para $q=3$), então a divergência observada
aqui tem uma explicação por recorrência determinística disponível além
do mecanismo de entropia da Proposição~\ref{prop:always-frozen}, e não
amostra por si só o ambiente genérico (não periódico) sobre o qual a
afirmação quenched realmente versa. Relatamos isso, portanto, como uma
ilustração do congelamento numa classe especial e facilmente computável
de raízes, não como confirmação num ambiente genérico; a
\S\ref{sec:kl-empirical} abaixo e a medição estatisticamente calibrada
da \S\ref{sec:kl} --- ambas amostrando muitas raízes deliberadamente
longe de qualquer ciclo curto conhecido --- carregam a evidência real
de ambiente genérico.
...
\begin{proposition}[Assíntota mais fina, condicional]\label{prop:transition-fine}
Condicional a um refinamento do tipo renovação, do nível da função de
contagem (não apenas o expoente) para a assíntota mais fina
$N(x)\sim W\cdot x^{\alpha_-(q)}$, valendo quase certamente e
uniformemente, \emph{inclusive} em $q=3$, para um fator de escala
aleatório $W$ q.c.\ positivo e não-degenerado (não uma constante
determinística em nenhum $q$, já que a lei de reprodução subjacente
tem variância genuína). Uma busca literária dirigida achou a
maquinaria vizinha, mas não este enunciado exato pronto: o clássico
teorema de renovação de Crump--Mode--Jagers \cite{Nerman1981} é
enunciado para processos de pontos de reprodução unilaterais
($[0,\infty)$), enquanto nossos deslocamentos de ramificação incluem
valores negativos (e.g.\ $2/3<1$ em $q=3$, já que um filho pode ser
numericamente menor que seu pai); a teoria de martingale de passeio
aleatório ramificado, nativamente bilateral \cite{Biggins1992}, dá
convergência geração-por-geração, não uma lei de renovação somada
sobre todas as gerações; e a teoria de renovação implícita para
árvores ponderadas com pesos gerais (bilaterais)
\cite{Goldie1991,JelenkovicOlveraCravioto2012,AlsmeyerBigginsMeiners2012}
resolve a \emph{cauda} da equação de ponto fixo para o próprio $W$, não
a lei de contagem cumulativa para $N(x)$ --- uma pergunta relacionada
mas distinta. Esperamos que o enunciado presente siga de combinar essa
maquinaria (decomposição de espinha e basculamento exponencial para um
passeio aleatório unidimensional bilateral, exatamente como já usado
nos resultados de cauda-de-$W$ citados) mas não realizamos essa
adaptação, nem a localizamos já escrita exatamente nesta forma; um
preprint muito recente \cite{VillemonaisZalduendo2025} estende leis de
convergência de processos de ramificação a espaços de tipo gerais sem
reescala geométrica e pode já contê-la, mas não verificamos isso. A
hipótese não-reticulada que qualquer extensão de \cite{Nerman1981}
precisaria vale aqui, e é exatamente a irracionalidade de $\log
q/\log2$ que a garante. Embora cada deslocamento de um passo $-\log q
+ a\log2$ ($a\in\mathbb{Z}_{\ge1}$) esteja contido em um único coset
do reticulado $\log(2)\,\mathbb{Z}$, estar contido em um coset não é
aritmeticidade: o menor subgrupo fechado de $\mathbb{R}$ gerado por
esses deslocamentos é $\log(q)\,\mathbb{Z}+\log(2)\,\mathbb{Z}$, que é
denso em $\mathbb{R}$ para todo $q\ge3$ ímpar porque $\log
q/\log2\notin\mathbb{Q}$. Equivalentemente, a função característica do
deslocamento vale $1$ apenas na origem, de modo que a assíntota de
renovação/CMJ não carrega modulação log-periódica (período $\log2$).
Se $\log q/\log2$ fosse racional, as posições dos ramos colapsariam
sobre um reticulado genuíno e apareceria uma correção periódica; a
irracionalidade é, portanto, o fato operante, não um fato
irrelevante.
\end{proposition}
...
e da Proposição~\ref{prop:transition-fine}: para $q=3$ ela tem
densidade natural positiva ($N_u(x)\sim x$); para
todo $q\ge5$ ímpar ela tem densidade natural nula, com
$N_u(x)\sim W_u\cdot x^{\alpha_-(q)}$. Em $q=3$ isto \emph{implica, e
refina estritamente}, a Conjectura do Expoente de Crescimento de
Kontorovich--Lagarias \cite{KontorovichLagarias2009} e
Applegate--Lagarias
\cite{ApplegateLagarias1995I,ApplegateLagarias1995II} para a árvore
reversa clássica de Collatz, que afirma apenas o expoente
$\pi_a(x)=x^{1+o(1)}$, não uma constante assintótica específica ---
aberta desde 1995, cujo melhor resultado incondicional parcial
permanece sendo a cota unilateral $\pi_a(x)\ge x^{0{,}84}$ de
Krasikov--Lagarias \cite{KrasikovLagarias2003}. Nossa identidade de
pressão reproduz a afirmação de expoente-$1$ dessa conjectura como o
caso $\alpha_-=1$ de uma única equação fechada, junto com seu análogo
para $q\ge5$, em vez de resolvê-la; a alegação adicional de uma
constante limite específica é conteúdo extra da
Conjectura~\ref{conj:transition-arithmetic} além da conjectura
clássica.
\end{conjecture}

Enunciamos o Teorema~\ref{thm:transition-model}, a
Proposição~\ref{prop:transition-fine}, e a
...
análogo é conhecido por nós: a lacuna é a mesma já identificada perto
do Teorema~\ref{thm:barrier} (Observação~\ref{rem:barrier-reading}:
quase-independência de dígitos frescos entre subárvores irmãs, aqui
numa forma quantitativa de grandes desvios em vez de qualitativa), e o
resultado aritmético genuíno mais próximo na
...
Observação~\ref{rem:transfer-basis}. A mesma relação sobrevive
inalterada quando $2$ não é raiz primitiva mod $q$ (as classes
estéreis extras da \S\ref{sec:setup}): restrita aos tipos
sobreviventes $\langle2\rangle$, a matriz de pressão multitipo tem
posto $1$, e seu autovalor de Perron colapsa, por um cancelamento
independente de $d$ ($a_0$ é bijeção $\langle2\rangle\to\{1,\ldots,
d\}$), para exatamente $q^{s-1}/(2^s-1)$ --- a mesma equação de
pressão --- então tanto o expoente quanto a direção da família de
escala sobrevivem intactos; verificado para $q=7,15$ a $1$--$4\%$.
(Se $2$ é raiz primitiva mod $q$ é, para $q$ primo, governado pela
conjectura da raiz primitiva de Artin, aberta em geral mas provada
condicionalmente à HRG por \cite{Hooley1967}.)}
...
\texttt{collatz-endogeny/\allowbreak sec3-pressure-equation}) aplicou uma bateria
...
(\texttt{collatz-endogeny/\allowbreak sec3-pressure-equation}):
...
\begin{theorem}[A recursão não força endogenia]\label{thm:barrier}
O modelo de transformação de suavização associado à recursão exata de
pesos da árvore reversa admite soluções não-endógenas (fatores de
gauge harmônicos, Proposição~\ref{prop:gauge}; a obstrução $2$-ádica é
excluída pelo Lema~\ref{lem:2adic}) com piso de variância condicional
$\sigma_Y>0$ estritamente positivo em toda resolução, estatisticamente
indistinguíveis da solução endógena por pressão, índice de cauda, ou
qualquer marginal que testamos. Consequentemente, a recursão exata de
pesos junto com as estatísticas dos dígitos $3$-ádicos não força, por
si só, a dispersão residual de gauge a se anular.
\end{theorem}

\begin{remark}[Leitura de barreira, e status epistêmico]\label{rem:barrier-reading}
Informalmente isso delimita uma classe de argumentos: não conhecemos
nenhum argumento que use apenas \textup{(i)} a recursão exata de pesos
e \textup{(ii)} estatísticas dos dígitos $3$-ádicos que não fatore
através deste modelo de transformação de suavização, e portanto nenhum
que possa distinguir a solução endógena da não-endógena. Nós \emph{não}
formalizamos ``usa apenas (i) e (ii)'' como uma classe precisa de
técnicas (em contraste com barreiras de relativização ou natural
proofs em teoria da complexidade), então isso é uma delimitação
heurística de abordagens conhecidas, não um teorema formal de
impossibilidade. A exclusão $2$-ádica (Lema~\ref{lem:2adic}) e a
construção de gauge (Proposição~\ref{prop:gauge}) sobre as quais o
Teorema~\ref{thm:barrier} se apoia são totalmente rigorosas e
verificadas independentemente; a alegação de classificação acima (que
qualquer tal argumento fatora através do modelo de transformação de
suavização) pode ser enunciada incondicionalmente como ``maquinaria
padrão sob [hipóteses], verificação detalhada pendente'', em vez de
independentemente re-derivada contra nosso setup exato. Nunca afirmamos
que a dispersão residual de gauge $\sigma_Y$ se anula para o objeto
aritmético verdadeiro; só que a recursão sozinha não pode forçar isso.
O ingrediente faltante que fecha a lacuna \emph{no modelo idealizado}
--- e que resta transferir para o cenário aritmético --- é a
quase-independência, entre subárvores irmãs, dos dígitos $3$-ádicos
frescos dos inteiros-folha, junto com equidistribuição da fase
arquimediana ao longo dos caminhos. O análogo rigoroso conhecido, na
direção forward, é o decaimento de Fourier das variáveis de Syracuse
de Tao (Proposição~\ref{prop:tao117}); a versão árvore-reversa/densidade
...
de apenas deixar de contradizer a conclusão. Declaramos o escopo desta
cota com precisão: nenhum acoplamento da forma funcional específica
explorada pelo Braço~2 (replicação de família inteira dos
\emph{conteúdos} de subárvore em intensidade $\rho$) foi detectado na
árvore real além de $\rho_{\mathrm{eff}}\lesssim0{,}06$ da intensidade
máxima dessa família. Isso não é, por si só, uma cota sobre toda forma
concebível de acoplamento; em particular não limita a correlação
exata, grosseira, dependente de $\Delta$, de par da
Proposição~\ref{prop:exact-endogeny} abaixo, que vive num eixo
disjunto (resíduo da raiz, não conteúdo da subárvore) já presente
identicamente, por construção, no próprio Braço~1 --- então não é uma
forma de acoplamento que esta calibração poderia ter detectado ou
deixado de detectar. Veja a Observação~\ref{rem:calibration-consistency}
para o argumento estrutural verificado. A equação~\eqref{eq:rho-eff}
converte a lacuna puramente
teórica do Teorema~\ref{thm:barrier} numa cota empírica quantitativa e
falsificável do seu tamanho, para esta família de acoplamentos.
...
$\eta^\ast_{5,BP}\approx 0{,}678$. Os próprios autores de
\cite{KontorovichLagarias2009} sinalizam isso como uma
...
\begin{theorem}[O regime 3 é ao menos tão difícil quanto problemas abertos reconhecidos]
\label{thm:regime3}
O regime 3 acima é um caso especial de, e na generalidade necessária
aqui é ao menos tão difícil quanto, cada um de: (i) correlações de
...
\begin{remark}[Consistência com a calibração da \S\ref{sec:calibration}]
\label{rem:calibration-consistency}
A Proposição~\ref{prop:exact-endogeny} e o resultado nulo
$\rho_{\mathrm{eff}}\lesssim 0{,}06$ da \S\ref{sec:calibration} medem
objetos diferentes e não conflitam, embora o ponto mereça ser explícito.
A razão mais nítida é estrutural, não meramente quantitativa: no
modelo sintético subjacente aos Braços~1 e~2 da \S\ref{sec:calibration},
o tipo de um filho (seu resíduo mod $3$ --- exatamente o objeto
$Z_i$ em $\ell=1$ da Proposição~\ref{prop:exact-endogeny}) é calculado
como função determinística só da fase da raiz e do índice do ramo,
reproduzindo a estrutura cíclica real de admissibilidade (validada
contra inteiros verdadeiros), e é inteiramente independente da
intensidade de acoplamento $\rho$ que o Braço~2 varia: conferimos isso
diretamente no código do simulador (2000 sementes,
$\rho\in\{0,0{,}3,0{,}7,1\}$, zero divergências na sequência de tipos
resultante), e confirmamos que essa estrutura determinística de tipo
reproduz a correlação exata da Proposição~\ref{prop:exact-endogeny}
($+1$ para $\Delta\equiv0\pmod6$, $-1/2$ caso contrário) até a quarta
casa decimal. Em outras palavras, a correlação grosseira já está
presente, identicamente, no Braço~1 ($\rho=0$, o nulo ``sem
acoplamento'') tanto quanto na árvore real; $\rho$ governa, em vez
disso, um eixo disjunto --- se o \emph{conteúdo} das subárvores irmãs
(a aleatoriedade fresca que define o \emph{tamanho} de cada subárvore)
é compartilhado, não se seus resíduos de raiz são correlacionados. A
cota $\rho_{\mathrm{eff}}\lesssim0{,}06$ não poderia, portanto, ter
detectado, nem deixado de detectar, a correlação da
Proposição~\ref{prop:exact-endogeny} em primeiro lugar: essa
correlação nunca foi uma forma de acoplamento que a hipótese nula da
calibração remove. Uma razão secundária, quantitativa, reforça isso:
mesmo concedendo peso total à correlação grosseira, sua contribuição
para a variância \emph{agregada} que a calibração mede é diluída. A
covariância $C_{\mathrm{exato}}(\ell_0;\Delta)$ da
Proposição~\ref{prop:exact-endogeny} é finita e independente de $D$,
enquanto as médias agregadas $\mathbb{E}[Z_i]$ crescem
exponencialmente com a profundidade de truncamento, então sua
contribuição normalizada
$C_{\mathrm{exato}}/(\mathbb{E}[Z_1]\mathbb{E}[Z_2])\to0$ conforme a
profundidade cresce --- consistente com o excesso da
\S\ref{sec:calibration} decaindo monotonicamente a zero com a
profundidade de truncamento. Separadamente, a refutação da hipótese
$L^2$ abaixo (Teorema~\ref{thm:l2-refutation}) é a falha de uma
\emph{cota superior} sobre a covariância fina ($K_\infty=\infty$ torna
a cota acima vazia), não uma prova de que a covariância fina é de fato
grande; a \S\ref{sec:calibration} mede diretamente o acoplamento fino
que essa cota deixa sem controle, e o encontra pequeno. Por fim, os
pesos geométricos de ramo $q\cdot 2^{-a}$ concentram massa em gaps
$\Delta$ pequenos: ponderando pares de ramos por sua massa natural de
segundo momento $2^{-a_i}\cdot2^{-a_j}$ (a mesma ponderação que entra
no $\Cov$ acima), $\Delta=2k$ tem probabilidade $3\cdot4^{-k}$, dando
$\Pr[\Delta\equiv0\!\!\pmod6]=1/21$ contra
$\Pr[\Delta\not\equiv0\!\!\pmod6]=20/21$ exatamente, então a
correlação grosseira média sob esta ponderação natural é exatamente
$\mathbb{E}[\Corr]=\tfrac{1}{21}(+1)+\tfrac{20}{21}(-\tfrac12)=-\tfrac37$
(verificado em \texttt{collatz-endogeny/\allowbreak sec10-l2-refutation-and-jensen/}),
consistente com o sinal negativo observado do excesso residual na
\S\ref{sec:calibration}, embora (pelo ponto estrutural acima) nenhuma
correspondência quantitativa desse tipo seja de fato necessária para
reconciliar os dois resultados. As duas seções são, portanto,
complementares em vez de estarem em tensão: o Teorema~\ref{thm:barrier}
e a Proposição~\ref{prop:exact-endogeny} estabelecem que a recursão
\emph{não pode forçar} a decorrelação entre irmãos (a estrutura
grosseira por-par sobrevive em toda profundidade, em qualquer modelo
que respeite a regra de admissibilidade verdadeira), enquanto a
\S\ref{sec:calibration} estabelece que o resíduo agregado real, no
eixo ortogonal do conteúdo das subárvores, é mesmo assim
empiricamente pequeno.
\end{remark}
...
família de Erdős/Lagarias, incluindo o arcabouço geral de partes
fracionárias de Flatto--Lagarias--Pollington
...
(Teorema~\ref{thm:l2-refutation}): uma articulação adicional, dentro
do mesmo vocabulário de Fourier de segundo momento que a condição
$L^2$ (ambas decompõem via a soma de caracteres
$\Cov\propto\sum_\xi S_1(\xi)S_2(\xi)^\ast$ da \S\ref{sec:regimes}), do
mesmo ingrediente faltante --- não uma quarta ou quinta via
metodologicamente independente por si só.
\end{theorem}
...
\subsection{Um quinto vocabulário independente, testado empiricamente}
...
Isto dá um quinto vocabulário, tecnicamente não relacionado, para o
mesmo ingrediente faltante: equidistribuição/balanço de resíduos ao
longo da órbita verdadeira, aparecendo independentemente via a rota de
transformação de suavização/endogenia (\S\ref{sec:endogeny}), a
contagem de entropia compartilhada pela Weak Covering Conjecture e por
$\beta=1$ (\S\ref{sec:wcc}, \S\ref{sec:triangulation} --- provadas o
mesmo objeto algébrico pela Proposição~\ref{prop:beta-wcc}, portanto
contadas uma vez), a equação funcional de Wirsching de 2003
(\S\ref{sec:triangulation}), a análise de Fourier de segundo momento
compartilhada pela condição $L^2$ e pela dicotomia espectral da
Proposição~C (\S\ref{sec:lemmas}, igualmente contadas uma vez), e agora
uma reformulação puramente combinatória.
...
Segundo, esses resultados não estão isolados: a mesma afirmação
aritmética subjacente --- alguma forma de equidistribuição ou
quase-independência de dígitos $3$-ádicos ao longo da dinâmica inteira
verdadeira e determinística, não apenas ao longo de um modelo i.i.d.\
idealizado --- é o cerne alcançado por cinco vocabulários
metodologicamente independentes: (1) a rota de transformação de
suavização/endogenia (\S\ref{sec:endogeny}); (2) a contagem de entropia
compartilhada pela Weak Covering Conjecture de Wirsching e por
$\beta=1$ de Tao, que a Proposição~\ref{prop:beta-wcc} mostra ser o
\emph{mesmo objeto algébrico}; (3) a equação funcional real de
Wirsching de 2003 e sua densidade de Fabius (\S\ref{sec:triangulation});
(4) a análise de Fourier de segundo momento subjacente tanto à condição
$L^2$ quanto à dicotomia espectral da Proposição~C
(\S\ref{sec:lemmas}), que compartilham a decomposição comum de
caracteres $\Cov\propto\sum_\xi S_1(\xi)S_2(\xi)^\ast$; e (5) a
reformulação puramente combinatória de balanço de resíduos de
\cite{Chang2026}. Contamos cinco em vez de sete deliberadamente: dois
dos vínculos par-a-par não são analogias, mas \emph{identidades
provadas} --- $\beta=1\cong$ WCC (Proposição~\ref{prop:beta-wcc}) e a
origem comum de segundo momento dos ramos $L^2$ e dicotomia-espectral
--- e contamos cada par uma vez. Que dois vocabulários possam ser
\emph{provados} como um único objeto é, por si só, evidência mais forte
de que a obstrução é uma única característica estrutural do problema
do que uma analogia solta seria. Uma barreira visível só de um ângulo é
candidata a artefato daquela técnica; uma barreira alcançada por cinco
vocabulários independentes (pontos fixos de processo de ramificação,
contagem de cobertura/entropia inteira, uma equação funcional real,
somas de caracteres de segundo momento, e contagem elementar de
resíduos), duas de cujas coincidências par-a-par são teoremas, é
evidência muito melhor de que a obstrução é uma característica
estrutural do problema em si.
...
parede a partir de cinco ângulos independentes, e um resultado
empírico duro e incondicional obtido no caminho.
...
\item \textbf{Quase-independência de dígitos $3$-ádicos frescos entre
  subárvores irmãs} (Teorema~\ref{thm:barrier},
  Observação~\ref{rem:barrier-reading}): a afirmação
...
\item \textbf{A transição de densidade na árvore aritmética genuína}
  ...
  (Teorema~\ref{thm:transition-model}, e, condicionalmente,
  Proposição~\ref{prop:transition-fine}); em $q=3$ esta conjectura
  implica, e refina estritamente, a Conjectura do Expoente de
  Crescimento de Kontorovich--Lagarias e Applegate--Lagarias, aberta
  desde 1995, cujo melhor resultado incondicional parcial permanece
  sendo a cota unilateral de Krasikov--Lagarias
  \cite{KrasikovLagarias2003}.
\end{enumerate}
...
\section*{Declaração de Disponibilidade de Dados de Pesquisa}
...
O autor declara não haver conflito de interesses relativo à publicação
deste artigo.
...
numérica e preparação do manuscrito, seguindo o que acreditamos ser a
melhor prática na pequena literatura existente sobre contribuições
