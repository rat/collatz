...
Condicional à extensão padrão do teorema de renovação de
Crump--Mode--Jagers \cite{Nerman1981} a medidas de reprodução gerais,
não necessariamente unilaterais --- cujas hipóteses não verificamos em
detalhe aqui, dado que nossos deslocamentos de ramificação incluem
valores negativos (e.g.\ $2/3<1$ em $q=3$, já que um filho pode ser
numericamente menor que seu pai) --- a assíntota mais fina
$N(x)\sim W\cdot x^{\alpha_-(q)}$ vale quase certamente, uniformemente e
\emph{inclusive} em $q=3$, para um fator de escala aleatório $W$ q.c.\
positivo e não-degenerado (não uma constante determinística em nenhum
$q$, já que a lei de reprodução subjacente tem variância genuína). A
própria hipótese não-reticulada de \cite{Nerman1981} vale aqui, já que
$\log q/\log2$ é irracional para todo $q\ge3$.
\end{proposition}
...
$\Delta$ pequenos, predominantemente no caso $\Delta\not\equiv
0\pmod 6$ (correlação grosseira $-1/2$) em vez de $\Delta\equiv
0\pmod 6$ (correlação grosseira $+1$), então a contribuição grosseira
líquida é plausivelmente \emph{negativa} --- consistente com, embora
paremos aquém de afirmar que prevemos, o sinal negativo observado do
excesso residual na \S\ref{sec:calibration}. As duas seções são,
