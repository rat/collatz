...
mais difícil que o caso $q=3$ já resolvido na \S\ref{sec:regimes}, e
resolvê-la provavelmente exigirá uma comparação pré-registrada contra
modelos alternativos de cauda (em particular um ajuste log-periódico,
já que os pesos dos ramos são potências de $2$) com um orçamento
computacional muito maior do que a enumeração populacional exaustiva
permite; um controle analítico sobre o espectro do operador de
transferência linear, uma vez obtido, revelou-se não relevante para
esta questão, pois o transiente vive numa camada não-linear separada.
Listamos isso como problema em aberto explícito na
...
\item \textbf{O índice de cauda do fator de escala do martingale}
  ...
  estatisticamente não-confirmatória, uma bateria completa de quatro
  estimadores (\S\ref{sec:freezing}) dá um quadro misto, não
  confirmatório, e um teste exato de momento populacional na mesma
  seção é inconclusivo na profundidade que a enumeração exaustiva
  alcança, por um motivo específico e identificado (um transiente de
  decaimento lento conhecido). Esta é, honestamente, a alegação com
  apoio mais fraco do paper, e a sinalizamos como tal em vez de
  deixá-la herdar credibilidade do caso $q=3$ só por compartilhar a
  fórmula.
\end{enumerate}
