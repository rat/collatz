...
occur --- two in a seven-paper program by a single prolific author, one
in an otherwise careful standalone result --- but they cluster
differently: the prolific author's errors are both statement-versus-proof
inconsistencies contained to a single section each, while the
...
in a Seymour working paper, one a pair of citation-only slips in
