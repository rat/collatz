...
$\varphi$, zero truncation error at every point tested,
...
$4\times10^{-4}$ under the sub-range and model checks tested.
\end{abstract}
...
\cite[Proposition~9.1]{BergKruppel1998} for a particular solution
...
is, uniformly for $|k-\ell|\le\delta_1\sqrt\ell$, at most
$\exp(-c\sqrt\ell+O(\log^2\ell))$, for a tail-window radius $\tau>0$
unrelated to the lower-bound constant $\eta$ of
\S\ref{sec:microcanonical} below. For $k$ in that window and
$m<\tau\sqrt\ell$, $|k-m-\ell|\le(\delta_1+\tau)\sqrt\ell$; choosing
$\delta_1+\tau<\delta$ puts $j=k-m$ inside the window of $(\star2)$,
...
$\delta_1+\tau<\delta$ that closes the implication, checked
...
$3^\ell$, the pushforward of the uniform measure on the reverse
Collatz tree at depth $\ell$ onto residues \cite{Tao2020}
...
a rate close to the minimum's own, even though the quantile itself sits
...
gives $\mathcal S_3(3,a)=9/7$ for
$a\equiv1\pmod3$ and $12/7$ for $a\equiv2\pmod3$, summing to $3$, not
$2$, so the Haar average of $\mathcal S_\ell(k,\cdot)$ over $a$ is
$3/2$; the
...
the range ($\ell\ge350$) leaves $L$ unchanged to six digits; adding a
further $1/\ell$ term moves it by $4\times10^{-4}$. The
...
Result~\ref{thm:fixed-precision-finite}; the negative Fourier-based
lower bound of Empirical Result~\ref{thm:microcanonical-fourier}
confines termwise absolute summation to the same finite-level regime.
