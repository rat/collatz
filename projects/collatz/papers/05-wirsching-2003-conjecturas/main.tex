...
measurement cannot by itself distinguish a genuine constant from a
...
analogous class of sequences $(x_\ell)\in\mathbb I_3^{\mathbb N}$ on
the continuous state space $\mathbb X$ above, used in condition
...
five-constant closed form of the shape
$t^{(\cdot)}(-\ln t)^{(\cdot)}\exp(-(\cdot)\ln^2(t/(-\ln t)))$
whose constants, evaluated at $a=3$, $\lambda=2/3$ (their own
notation, not reused here to avoid clashing with $\delta$ elsewhere in
this paper), are reproduced verbatim in the accompanying repository
...
a rate close to the minimum's own (at $d=+12$, the minimum over all
units falls from $0.3840$ at $\ell=10$ to $0.2378$ at $\ell=17$, a
rate of $0.93$ per level, matching the quantile's own $0.93$ above),
even though the quantile itself sits
well above the extreme order statistic, the single minimum. Whether it
...
two-values-per-level pattern holds at
...
Conjecture 3 asks for a genuine constant $c$; Berg and Kr\"uppel's own
...
satisfy. $L_\ell$ cannot distinguish the two: at $u=0$,
$\log_3x_{3\ell}^+-\log_3x_\ell^+$ is the integer $1-2\ell$, so
$Q(x_{3\ell}^+)=Q(x_\ell^+)$ exactly, and the deficit above tests the
$\varphi_0$-only ratio regardless of $Q$. A periodic $Q$ would instead
pin $\ln(\varphi/\varphi_0)$ to the same value at every such
same-phase pair $(\ell,3\ell)$, with zero drift. The measured drift is
nonzero and tracks the $\varphi_0$-based fit: $+0.0320$ observed at
$(100,300)$ against $+0.0335$ predicted, and $+0.0271$ at $(150,450)$
against $+0.0273$
predicted, agreement to $4\%$ and $1\%$; the shallow pair $(10,30)$
disagrees badly, consistent with the fit not yet applying there. This
disfavors a dominant periodic factor. It does not rule out a small
one, nor prove $c$ constant.
\end{remark}
...
it applies, but $L_\ell$ alone cannot separate a genuine limit constant
from a periodic factor; Remark~\ref{rem:phase-check} adds the
same-phase check that does, and disfavors the periodic alternative.
