...
point tested, supports Conjecture 3 at every offset tested,
...
$a=3$, $\lambda=2/3$ (their own notation, not reused here to avoid
...
Berg and Kr\"uppel
expect $\varphi/\varphi_0$ to stay bounded and bounded away from zero,
not necessarily to converge
...
equicontinuous bounded families. Neither bridge is uniform in growing
...
N$, non-cyclic, $a\not\equiv0\pmod3$, while conditions $(\star2)$ and
$(\star3)$ are stated over $\mathbb Z_3^\times$.
...
in the window for every $\delta>0$, a zero at $k=\ell$ recurring at
arbitrarily large $\ell$ would falsify this hypothesis and both
$(\star2)$ and $(\star3)$ for every $\mu_1,\eta>0$: a $\liminf$-type
...
Thus the fixed-cost law does not approach the uniform law on units at
...
not, and not even on Haar average: since every $j$ is admissible for
exactly one residue class of $a$ mod $3$, summing the recursion over
both classes gives $\sum_{j<c_{\ell-1}}\overline
g_{\ell-1}(k-j)=3\overline g_\ell(k)$ for every $(\ell,k)$, so
...
would vanish identically at every such pair. It does not, but it moves
toward the value the smooth fit itself predicts as $\ell$ grows, not
toward zero: observed drift $-0.013$, $+0.039$, $+0.032$, $+0.027$
against fit-predicted $+0.106$, $+0.047$, $+0.034$, $+0.027$ at
$(\ell,3\ell)=(10,30),(50,150),(100,300),(150,450)$, a relative
mismatch of $112\%$, $18\%$, $4\%$, and $1\%$. That is the signature of
a shrinking finite-size correction to $\varphi_0$, not of an
uncontrolled periodic contamination, which would leave a mismatch of
fixed size at every depth. This disfavors a dominant periodic factor
