...
its annealed pressure, via an exact fibre-counting bijection,
correcting a finite-automaton argument for the same identity that we
show is not well posed. The nontrivial root of
$\rho_{\mathrm{ann}}(\alpha)=1$ undergoes a structural transition at
$q=5$: it equals exactly $\alpha^\ast=2$ at $q=3$ and falls strictly
below $1$ for every $q\ge5$. For $q=3$ this is the classical Growth
Exponent Conjecture of Kontorovich--Lagarias and Applegate--Lagarias.
We show that the two roots of this equation always sit on opposite
...
quenched counting function of the matching i.i.d.\ branching-random-walk
model; the larger root is always frozen, so the pressure identity
alone does not establish the tail index of the density martingale. We
prove that the tilted $q$-adic measure is singular at every frozen
root by an entropy identity and a fractional-moment estimate, and we
prove the tail index for the matching i.i.d.\ model by an implicit
renewal theorem. For the coherent $q$-adic environment we prove
martingale convergence and identify the limit with the density of the
absolutely continuous component; absolute continuity and the tail
remain conjectural. At $q=3$ the predicted index $\alpha^\ast=2$
matches independent empirical measurements; for $q\ge5$ the claim is
strengthened by a large-sample statistical test at $q=5$, though still
not proved.
\end{abstract}
...
exact bijective counting argument rather than a heuristic transfer
operator, a closed-form equation for the annealed growth rate of the
...
relative position undergoes a genuine structural change at $q=5$ that
...
singularity at frozen roots). \S\ref{sec:freezing} develops the
freezing transition. \S\ref{sec:transition} states the structural
transition at $q=5$ precisely, separating what is proved for the
matching stochastic model from what remains conjectural for the
genuine arithmetic tree. \S\ref{sec:tail-regime} classifies the root
...
\]
The larger root is frozen, hence $H(p_\alpha)<\log q$ (see
...
$\rho_{\mathrm{ann}}=e^P$ is log-convex (\S\ref{sec:tail-regime}),
...
fails to track $\rho_{\mathrm{ann}}(1)=1$. This check has a
limitation, flagged rather than hidden: every one of these
...
roots, not as a confirmation at a generic one; \S\ref{sec:empirical}
below carries the actual generic-environment evidence, averaging over
many roots sampled well away from any known short cycle.
...
$\alpha>0$, discussed in \S\ref{sec:tail-regime} below, the
...
of this line of work. For $q\ge 5$, however, the second root falls
...
rather than resolving it. A calibrated empirical test of this
...
error corrected in Example~\ref{ex:naive-fails};
Remark~\ref{rem:transfer-basis} below makes precise what does and does
...
For the genuine i.i.d.\ branching-random-walk model that the annealed
...
is known to us: the closest genuine arithmetic result in
...
$q=3,5,7,9,15$ in the accompanying repository.
...
theory on block maxima (both from earlier stages of this line of work)
give $\alpha\approx2$ independently of the pressure argument, and the
...
$1.536\ldots$) was later shown, over the course of this line of work,
to be statistically non-confirmatory at that sample size (the
estimator's true standard error is $\approx0.45$, an order of magnitude
larger than the agreement being claimed). We pursued this further along
two independent routes, and report both honestly rather than stopping
...
log-normal alternative). The outcome is mixed rather than confirmatory:
the bias-corrected Hill estimator is remarkably stable across headroom
...
original under-powered measurement, and both point to the same
...
measurements noted above. We have since pursued exactly the two
follow-ups this pointed to. A pre-registered log-periodic fit
...
follow-up produced a real gain: a pre-registered comparison on a
...
This is not a coincidence alongside the freezing computation of
...
\section{Precise regime of the tail exponent at \texorpdfstring{$q=3$}{q=3}}
\label{sec:tail-regime}

A directed literature search raised the question whether
$\alpha^\ast=2$ (the second root at $q=3$) coincides with the
\emph{critical case} of the classical multivariate smoothing-transform
theory \cite{KoleskoMentemeier2015}, characterized by $m'(\alpha)=0$ at
a root of $m(\alpha)=1$ (a boundary/degenerate case in the sense of
...
\cite{BarrierCompanion} --- so even setting aside the regime mismatch,
...
arithmetic recursion --- which is exactly the question
Proposition~\ref{prop:always-frozen} and Conjecture~\ref{conj:tail-index}
address directly.
...
$w_2(1)=1$ is a genuine fixed point), the smallest-branch map
