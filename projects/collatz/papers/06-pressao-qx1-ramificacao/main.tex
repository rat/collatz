...
Proposition~\ref{prop:always-frozen}). The numbers
$p_\alpha(a):=q^{\alpha-1}2^{-\alpha a}$ of \S\ref{sec:pressure} form a
probability law of mean $\mu$; let $\xi_1,\xi_2,\ldots$ be i.i.d.\ draws
from it. Fix $0<\varepsilon<\mu$ and $L\ge1$, and let $Y_L$ be the
...
measurable order): this defines a genuine sub-population, resampled
...
construction, at least one copy survives almost surely; the almost
surely finite depth and displacement at which that copy is rooted only
shifts what follows by a bounded amount, negligible in the limits
below. Every one of the $Z_j^{(K)}$ retained individuals lies at
generation $jL$ with displacement at most $jL(\mu+\varepsilon)$, so
$N\bigl(e^{jL(\mu+\varepsilon)}\bigr)\ge Z_j^{(K)}$, and monotonicity
of $N$ (the exponentially spaced points $jL(\mu+\varepsilon)$ have
consecutive ratio $\to1$) gives
\[
...
almost-sure events), so the right side may be replaced by its supremum
over $K$, namely $\log r_L/(L(\mu+\varepsilon))$. Letting
$L\to\infty$ and then $\varepsilon\downarrow0$, using
$\log r_L\ge\alpha L(\mu-\varepsilon)+\log\Pr_\alpha(\cdots)\to\alpha
L(\mu-\varepsilon)(1+o(1))$, gives
...
2^{-\theta}(1+5^{\theta-1})$, not $\rho_{\mathrm{ann}}(\theta)$, so the
two models are not the same stochastic process. They do share every
root: $m_{\mathrm{KL}}(\theta)-1=\bigl((2^\theta-1)/2^\theta\bigr)
