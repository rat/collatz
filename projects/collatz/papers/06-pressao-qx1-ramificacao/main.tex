...
its annealed pressure, via an exact fibre-counting bijection. The
nontrivial root of $\rho_{\mathrm{ann}}(\alpha)=1$ undergoes a
structural transition at $q=5$: it equals exactly $\alpha^\ast=2$ at
$q=3$ and falls strictly below $1$ for every $q\ge5$. For $q=3$ this
is the classical Growth Exponent Conjecture of Kontorovich--Lagarias
and Applegate--Lagarias. We show that the two roots of this equation
...
continuity and the tail index remain conjectural. At $q=3$ the
predicted index $\alpha^\ast=2$ matches independent empirical
measurements. At $q=5$, the largest of three statistical tests run at
increasing sample size gives evidence consistent with the conjectured
tail index, though smaller-sample tests were inconclusive and the
claim remains unproved for every $q\ge5$.
\end{abstract}
...
$q=9=3^2$ the sterile classes are $u\equiv0,3,6\pmod9$, not $u\equiv0$
alone. In general $d=\ord_q(2)$ can also be a proper divisor of
...
$2^a/q$ from its parent, matching the arithmetic tree's own transition
rule (\S\ref{sec:setup}) one branch at a time rather than only in
aggregate. Writing $S_v\in\mathbb{R}$ for the cumulative log-displacement
...
same pressure function as Theorem~\ref{thm:pressure}, not by a separately
chosen one.
...
\emph{Lower bound.} This direction, matching the upper bound exponent
almost surely on the survival event, is the content of
Kontorovich--Lagarias's Theorem~8.10 \cite{KontorovichLagarias2010},
proved there for the single-type $5x+1$ branching random walk by a
large-deviations argument adapted from Lagarias--Weiss (the same
Chernoff computation as above, run in reverse to locate the generation
$k$ at which the bulk of the count accumulates, together with a
second-moment estimate showing this generation's contribution is not a
rare event). Their argument uses only the branching structure's pressure
function, the independence of fertility across nodes, and survival on
the large-deviation event with positive probability, the last of which
is exactly what unfreezing at $\alpha_-(q)$
(Proposition~\ref{prop:always-frozen}) supplies; with that in hand the
argument transfers without change, and we do not repeat the
second-moment computation here.
\end{proof}

\begin{proposition}[Finer asymptotic, conditional]\label{prop:transition-fine}
The counting function conjecturally satisfies the sharper asymptotic
...
of $x^{\alpha_-(q)}$, not merely the exponent, which needs machinery
beyond the large-deviations argument used above. A directed literature
...
not one theorem about the arithmetic tree, because the step
from one to the other is exactly the kind of transfer that
...
change its shape.
Verified on $5000$ sampled roots across four headroom levels (the
truncation bound $n_{\max}=v\cdot H$ expressed as a multiple $H$ of
each root $v$, rather than an absolute size), with
...
$\langle2\rangle\to\{1,\ldots,d\}$), to exactly $q^{\sigma-1}/(2^\sigma-1)$, the
same pressure equation, so both the exponent and the
...
direct enumeration of \S\ref{sec:empirical}
...
distinct claim from the tail index tested below). For $q\ge5$ the
tail-index evidence tested here is markedly weaker: an
...
larger than the agreement being claimed). Two further tests bear on
this question.

First, a larger statistical test (5000 roots, four
...
Neither route confirms Conjecture~\ref{conj:tail-index}
...
measurements noted above. Two follow-ups address this. A log-periodic
...
provably perfect spectral gap). The second follow-up produced a real
gain: a comparison on a $20\times$ larger sample ($10^5$ roots)
