...
Applegate--Lagarias. The two roots of this equation always sit on opposite sides of a
...
The annealed identity then transfers to the quenched counting function
of a matching i.i.d.\ branching-random-walk model. The larger root is
always frozen. The pressure identity alone does not determine whether
the density martingale degenerates there. We prove that the tilted $q$-adic measure is
...
handful of roots at $q=5,7$ chosen well away from any known short
cycle.
...
and twofold: the Hill estimator and extreme-value theory on block
...
argument. The direct enumeration of \S\ref{sec:empirical}
...
still relaxing toward its asymptotic exponent. This is not an effect of
the linear transfer operator's spectrum, which we verified to be
exactly $\{\Lambda,0\}$ at every truncation level, a perfect spectral
gap with no isolated subdominant eigenvalue; the transient lives in a
separate, nonlinear critical layer and remains open. A power-law fit
...
case}, characterized by $m'(\alpha)=0$ at a root of $m(\alpha)=1$, the
theory's boundary or degenerate case \cite{KoleskoMentemeier2015}. Writing
