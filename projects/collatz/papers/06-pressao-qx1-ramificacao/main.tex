...
transition at $q=5$. We prove this exponent statement for a matching
i.i.d.\ branching model; at $q=3$ it recovers a 1992 result of
Lagarias--Weiss. The same statement for the actual arithmetic reverse
tree is the classical Growth Exponent Conjecture of
Kontorovich--Lagarias and Applegate--Lagarias, which we do not
resolve. The two roots of this equation always sit on opposite sides of a
quenched/annealed freezing transition, in the sense of directed
polymers in a random environment. The smaller root is always unfrozen,
and the larger root is always frozen. At the unfrozen root the
annealed identity transfers to the quenched counting function of a
matching i.i.d.\ branching-random-walk model; at the frozen root the
pressure identity alone does not determine whether the density
martingale degenerates. We prove that the tilted $q$-adic measure is
...
relative position undergoes a structural change at $q=5$. The $q=3$
case reproduces, as a special instance, a $31$-year-old open conjecture
of Kontorovich--Lagarias and Applegate--Lagarias for the classical
Collatz map.
...
gives $F_{k+1}\equiv F_k\pmod{q^k}$), hence converge to a limit in
$\mathbb{Z}_q:=\varprojlim_k\mathbb{Z}/q^k\mathbb{Z}$, the $q$-adic
integers (the same projective-limit construction as for prime $p$,
applied here for composite $q$ too); write $\mu_\alpha$ for the limit's
law, the \emph{tilted Syracuse measure} at $\alpha$.
...
\[
 ...
 =-\alpha P'(\alpha),
\]
the quantity called $s(\alpha)$ once \S\ref{sec:freezing} defines it.
...
The freezing of the trivial root for every odd $q\ge5$ shares its sign
condition with the mean-drift remark below ($\log q-2\log2>0$): the
drift that makes orbits expand on average is, up to a minus sign,
exactly the entropy deficit that freezes $\alpha_+=1$.
...
special, easily-computed class of roots; we do not carry out the
matching check on generic, non-periodic roots at the frozen root here.
\S\ref{sec:empirical} below instead tests the growth exponent itself
(Conjecture~\ref{conj:transition-arithmetic}, governed by the
\emph{unfrozen} root) on a handful of roots at $q=5,7$ chosen well
away from any known short cycle, a related but distinct question from
this section's frozen-root divergence.
...
infinitely many children almost surely in the i.i.d.\ model (each
candidate exponent $a\ge1$ is, by construction, independently fertile
with probability $1/q$, so infinitely many independent Bernoulli$(1/q)$
trials give infinitely many successes almost surely, by the second
Borel--Cantelli lemma), and each spawns an independent copy of the whole
...
\begin{remark}[Relation to Kontorovich--Lagarias's Theorem~6.5, and to Lagarias--Weiss]
At $q=3$, Theorem~\ref{thm:transition-model}'s exponent-$1$ statement
is not new. Kontorovich--Lagarias \cite[Theorem~6.5]{KontorovichLagarias2010}
already prove $I^\ast(x;\omega)=x^{1+o(1)}$ almost surely for their
bounded-offspring branching-random-walk model $B[3^0]$ (at most two
offspring per node, again unlike the present model's almost-surely
infinite offspring), attributing the argument there to Lagarias and
Weiss \cite[Theorem~4.2]{LagariasWeiss1992}. The present proof, for a
structurally different model with infinite offspring, is independent
of theirs.
\end{remark}
...
Theorem~\ref{thm:transition-model}: it fixes the constant in front
of $x^{\alpha_-(q)}$, which needs machinery
beyond the exponential tilting and block truncation used above. The
neighboring machinery does not supply this exact statement ready-made:
the classical Crump--Mode--Jagers renewal theorem
...
odd $q\ge5$. At $q=3$ this is the Growth Exponent Conjecture, namely
$\pi_a(x)=x^{1+o(1)}$ for the classical Collatz reverse tree's analogue
of $N_u(x)$ above (the count, up to $x$, of integers reaching a
fixed root $a$ under repeated forward iteration of the map)
\cite[Conjecture~2.1]{KontorovichLagarias2010}, which restates in
exponent form the strictly stronger linear-growth conjecture of
Applegate--Lagarias, $\pi_a(x)\ge c_ax$ for a constant $c_a>0$
\cite[Conjecture~A]{ApplegateLagarias1995I}, open
...
quantile ratios matching this prediction to within $1$--$19\%$ across
the $36$ type-pair/quantile/headroom comparisons (most under $10\%$;
the loosest is the top-$20\%$ quantile for the type with smallest
predicted scale); exact
...
$0.2$--$4.2\%$. (Whether $2$ is a primitive root mod $q$ is, for prime
...
and comes by two independent methods on the same sample of roots: the
Hill estimator and extreme-value theory on block maxima, applied to
real reverse-tree samples for the classical $q=3$ Collatz map, give
$\alpha\approx2$ independently of the pressure argument (the two
estimators are not an independent replication in sample, only in
method). The direct enumeration of \S\ref{sec:empirical}
(Empirical Result~\ref{emp:real-trees}) is a separate, related line of
support at $q=5,7$: that enumeration targets
...
$q=5$ (600
roots, agreement to within $0.02$ of the predicted
$1.536\ldots$) is not statistically confirmatory at that sample size:
the estimator's true standard error is $\approx0.45$, more than an order of
...
levels at $1.606$--$1.614$, some $4$--$5\%$ above the predicted
$1.536$, with a $95\%$ confidence interval that still covers it; the
...
index climbing from $\approx1.3$ at the widest window to
$\approx2.4$ at deep, but not the deepest, quantiles, without settling
near any single value; the single deepest GPD threshold tested
(the top $1\%$) breaks this climb, dropping back to $1.66$, on an
excerpt with only $50$ exceedances and a correspondingly wide standard
error. Consistent with a slowly-converging pre-asymptotic regime in
...
$\alpha=\theta$, holds to within $2\times10^{-11}$ at $k=11$, the
residual truncation error of the finite cutoff $a\le60$ used in the
sum defining $Z_k$, not a floating-point limit, confirming the
...
toward its asymptotic exponent rather than one side having settled
while the other has not. This is not an effect of the \emph{dual}
operator $M_\alpha$, which pushes a parent residue's weight forward to
its children on the space of level-$K$ locally constant functions
(distinct from the Koopman operator of \S\ref{sec:pressure}, which
Example~\ref{ex:naive-fails} shows has no such finite-state form):
$M_\alpha$'s spectrum on this space is numerically $\{\Lambda,0\}$,
where $\Lambda:=q^\alpha/(2^\alpha-1)$ is its dominant eigenvalue, with
every other eigenvalue indistinguishable from a nilpotent block to
within $2\times10^{-4}$ at every truncation level $K\le4$ tested; the
transient lives in a separate, nonlinear critical layer and remains
open. A power-law fit
to the increment-ratio decay across $k=6,\ldots,11$ did not converge to
a stable exponent (the fitted rate varied by a factor of several across
nearby values of $p$, the signature of an underpowered fit at six
correlated points), so we do not have a reliable measurement of how
quickly the transient decays, and cannot say how much larger $k$ would
need to be to resolve it. We therefore
...
periodogram test found no power at the predicted frequencies. The dual
operator $M_\alpha$'s spectrum, once obtained numerically above,
likewise turned out not to bear on this question, since the transient
lives in a separate nonlinear layer and $M_\alpha$ already has a
spectral gap. The second follow-up extends the root
...
Vuong test that no longer favors the lognormal alternative; all three
were checked against two synthetic calibration tests. This is now the
...
At $q=3$, the second root $\alpha^\ast=2$ raises the question of
whether it falls under the \emph{critical case} of the classical
multivariate smoothing-transform theory, characterized there by
$\alpha\in(0,1]$ with $m'(\alpha)=0$ at a root of $m(\alpha)=1$
\cite{KoleskoMentemeier2015}. It does not, on two independent counts.
Writing $m(\alpha):=q^{\alpha-1}/(2^\alpha-1)$ (the pressure function
of \S\ref{sec:pressure} at $q=3$), we compute
\[
...
happen at $q=3$. $\alpha^\ast=2$ also lies outside the theory's stated
domain $\alpha\in(0,1]$ regardless. The correct classification
of $\alpha^\ast=2$ is therefore as the \emph{second root with
$m'>0$}: a Goldie-type regularly-varying tail regime
...
Goldie-type regime name correctly classifies the algebraic root
...
\section{Empirical measurements on real reverse trees}
\label{sec:empirical}

\begin{empirical}[Direct enumeration on real reverse trees]\label{emp:real-trees}
Exhaustive depth-first enumeration of the reverse tree, following the
exact admissibility rule of \S\ref{sec:setup}, up to $10^{10}$ from
roots $7,11,19$ at $q=5$ and up to $10^{12}$ from roots $9,11,15$ at
$q=7$, bears on Conjecture~\ref{conj:transition-arithmetic}. Excluding
the first and last logarithmic decade at each root (small-count noise
at the smallest checkpoints; systematic undercount at the largest, since
the traversal prunes an entire branch once some ancestor along it
exceeds the cutoff, even when a later descendant would fall back below
it through a sub-unit branch displacement, so nodes near the cutoff are
undercounted more than nodes well below it), the per-decade growth
slopes at $q=5$ span $0.56$--$0.70$ (predicted $\alpha_-(5)=0.650919$)
at all three roots. At $q=7$ (predicted $\alpha_-(7)=0.373501$), roots
$11$ and $15$ span $0.36$--$0.40$, consistent with the prediction, but
root $9$ stays at $0.26$--$0.34$ across every decade, including the
excluded ones (the two excluded decades are, if anything, further from
the prediction, at $0.27$ and $0.26$), a clear miss; its tree is also
some $40$ times smaller
at the cutoff ($2105$ nodes against $83466$ and $91878$ for roots $11$
and $15$), consistent with a root whose finite-depth behavior has not
yet entered the asymptotic regime the conjecture describes.
\end{empirical}
...
enumeration would loop on nodes that reach one. Since every node in the
reverse tree has a unique parent, no node outside a cycle is ever
revisited during the traversal; our implementation instead excludes
the cycle members themselves (precomputed for each $q$) from the
starting roots, instead of only sampling roots far from them.

\begin{remark}[Bibliographic calibration]\label{rem:novelty-109}
Neither the numerical value $\alpha^\ast(5)=0.650919$ nor the
qualitative transition at $q\ge5$ is new. For the analogous $p=2$
generalization of Tao's theorem, a later result proves the positive
direction, almost-bounded orbits, under a condition that reduces to
$q<4$ at $p=2$, and conjectures, with numerical support and no known
counterexample but no proof, that this condition is also necessary
for almost-bounded orbits, i.e.\ that almost-bounded orbits fail for
every $q\ge5$
...
\bibitem{KontorovichLagarias2010} A.\ V.\ Kontorovich and J.\ C.\
  ...
  problems and related problems}, in: \emph{The Ultimate Challenge:
  the $3x+1$ Problem} (J.\ C.\ Lagarias, ed.), American Mathematical
  Society, 2010, 131--188.

\bibitem{LagariasWeiss1992} J.\ C.\ Lagarias and A.\ Weiss, \emph{The
  $3x+1$ problem: two stochastic models}, Annals of Applied
  Probability \textbf{2}(1) (1992), 229--261.
