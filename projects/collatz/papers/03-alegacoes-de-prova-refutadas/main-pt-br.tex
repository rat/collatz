...
taxonomia de como elas falham: três categorias respondem juntas por dez
dos doze casos --- raciocínio circular que embute a conclusão da
conjectura numa definição ou premissa (três casos), generalização
ilegítima de uma única sequência adversária escolhida à mão para todas
as sequências (dois casos), e uma única falácia recorrente, tratar uma
propriedade agregada, média ou assintótica como uma garantia
determinística e pontual para uma trajetória específica, que sozinha
responde pelos cinco restantes (disfarçada respectivamente pelo Teorema
de Baker, por uma heurística de passeio aleatório, por um argumento de
Lyapunov 2-ádico, pela notação Big-O e pela densidade 2-ádica
estática). Os dois casos restantes são um erro elementar sem relação
com a dificuldade real da
...
a concentração: cinco dos doze casos, apesar de usar vocabulários
matemáticos inteiramente diferentes, cometem o erro idêntico de
confundir uma propriedade agregada ou assintótica com uma garantia
determinística e pontual, e mais cinco cometem um de dois erros
